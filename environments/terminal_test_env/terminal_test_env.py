...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Mercury Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Mercury Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Mercury Agent",
    },
    {
...
class TerminalTestEnv(MercuryAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/mercury-agent/.env.
        """
