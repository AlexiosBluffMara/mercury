...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs MercuryAgentLoop with terminal tool only
...
Requires: pip install mercury-agent[yc-bench]
"""
...
class YCBenchEvalConfig(MercuryAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends MercuryAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(MercuryAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The MercuryAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs MercuryAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our MercuryAgentLoop.
            ...
            # ----------------------------------------------------------
            # Step 2: Run the MercuryAgentLoop
            # ----------------------------------------------------------
