"""
MercuryAgentBaseEnv -- Abstract Base Environment for Mercury-Agent + Atropos

Provides the Atropos integration plumbing that all mercury-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via MercuryAgentLoop
...
# Ensure the mercury-agent repo root is on sys.path so that imports like
...
# Load API keys from mercury-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, MercuryAgentLoop
...
# Import mercury-agent toolset infrastructure
...
class MercuryAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for mercury-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of mercury toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: mercury, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/mercury-results/ "
...
class MercuryAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for mercury-agent Atropos integration.
    ...
    name: Optional[str] = "mercury-agent"
    env_config_cls = MercuryAgentEnvConfig

    def __init__(
        self,
        config: MercuryAgentEnvConfig,
        ...
        # Set terminal environment variables so mercury tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY mercury-agent tool (terminal, file, web,
