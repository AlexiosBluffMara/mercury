...
// Manual GLB loader — sidesteps useLoader + Suspense (which can stall the
// whole canvas if anything in the loader cache is unhappy).
  ...
  useEffect(() => {
    ...
    new GLTFLoader().load(
      ...
      (gltf) => {
        ...
        if (!g) { console.error("[Cortex] GLB has no Mesh:", url); return; }
        setGeom(g);
      },
      undefined,
      (err) => console.error("[Cortex] GLB load failed:", err),
    );
...
export function Cortex(props: CortexProps): JSX.Element {
  ...
  const geometry = useGLBGeometry(meshUrl);
