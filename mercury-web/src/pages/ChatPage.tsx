/**
 * ChatPage — embeds `mercury --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (mercury_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage() {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__MERCURY_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `mercury dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const token = window.__MERCURY_SESSION_TOKEN__;
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[mercury-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2 normal-case">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="mercury-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
...
declare global {
  interface Window {
    __MERCURY_SESSION_TOKEN__?: string;
  }
}
