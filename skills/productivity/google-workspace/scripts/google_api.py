#!/usr/bin/env python3
"""Google Workspace API CLI for Mercury Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Mercury-facing JSON contract and falls back to the Python client
...
from _mercury_home import get_mercury_home
...
def _gws_binary() -> str | None:
    override = os.getenv("MERCURY_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Mercury Agent")
