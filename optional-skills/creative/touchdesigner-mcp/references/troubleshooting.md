...
## 5. Mercury Configuration

### Config location

`$MERCURY_HOME/config.yaml` (defaults to `~/.mercury/config.yaml` when `MERCURY_HOME` is unset)
...
Restart the Mercury session for changes to take effect. The MCP connection is
