...
MERCURY_HOME_DIR="${MERCURY_HOME:-$HOME/.mercury}"
MERCURY_CFG="${MERCURY_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${MERCURY_CFG} manually"); }
    manual_steps+=("Restart Mercury session to pick up config change")
fi
