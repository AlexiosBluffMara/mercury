...
This module provides the glue:
    - ``MercuryTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Mercury Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses MERCURY_HOME so each profile gets its own OAuth tokens.
    Layout: ``MERCURY_HOME/mcp-tokens/``
    """
    try:
        from mercury_constants import get_mercury_home
        base = Path(get_mercury_home())
    except ImportError:
        base = Path(os.environ.get("MERCURY_HOME", str(Path.home() / ".mercury")))
...
# ---------------------------------------------------------------------------
# MercuryTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class MercuryTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        MERCURY_HOME/mcp-tokens/<server_name>.json         -- tokens
        MERCURY_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Mercury records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Mercury.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Mercury Agent")
    scope = cfg.get("scope")
