...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    mercury_node_bin = str(mercury_home / "node" / "bin")
    return [mercury_node_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-mercury_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed mercury processes — this way every clean mercury exit sweeps
    ...
    # Sweep orphans from other crashed mercury processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live mercury processes.
    try:
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning mercury process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced mercury PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent mercury instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-mercury_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live mercury process.
            continue
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Mercury-managed
    ...
    # Build an extended search PATH including Mercury-managed Node, macOS
...
def _run_browser_command(
    ...
    try:
        ...
        # Record this mercury PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = mercury_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    from mercury_constants import get_mercury_dir
    screenshots_dir = get_mercury_dir("cache/screenshots", "browser_screenshots")
