...
it is automatically downloaded from GitHub releases to $MERCURY_HOME/bin/tirith.
...
def _get_mercury_home() -> str:
    """Return the Mercury home directory, respecting MERCURY_HOME env var."""
    return str(get_mercury_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_mercury_home(), ".tirith-install-failed")
...
def _mercury_bin_dir() -> str:
    """Return $MERCURY_HOME/bin, creating it if needed."""
    d = os.path.join(_get_mercury_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $MERCURY_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_mercury_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $MERCURY_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $MERCURY_HOME/bin/tirith
    ...
    mercury_bin = os.path.join(_mercury_bin_dir(), "tirith")
    if os.path.isfile(mercury_bin) and os.access(mercury_bin, os.X_OK):
        _resolved_path = mercury_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return mercury_bin
