...
One ``CDPSupervisor`` runs per Mercury ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "mercury-dialog-bridge.invalid"
...
(() => {
  if (window.__mercuryDialogBridgeInstalled) return;
  window.__mercuryDialogBridgeInstalled = true;
  const ENDPOINT = "http://mercury-dialog-bridge.invalid/";
