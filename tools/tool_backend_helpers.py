...
def managed_nous_tools_enabled() -> bool:
    ...
    try:
        ...
        from mercury_cli.models import check_nous_free_tier
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.mercury/.env`` (via
    ``mercury_cli.config.get_env_value`` when available) so tool-side
