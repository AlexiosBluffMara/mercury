...
from mercury_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``MERCURY_PLATFORM`` or
            ``MERCURY_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.mercury/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.mercury/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    mercury = metadata.get("mercury") or {}
    if not isinstance(mercury, dict):
        mercury = {}
    return {
        "fallback_for_toolsets": mercury.get("fallback_for_toolsets", []),
        "requires_toolsets": mercury.get("requires_toolsets", []),
        "fallback_for_tools": mercury.get("fallback_for_tools", []),
        "requires_tools": mercury.get("requires_tools", []),
    }
