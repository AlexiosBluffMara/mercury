...
as ``kind: backend``) or ``~/.mercury/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``mercury tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``mercury tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``mercury tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$MERCURY_HOME/cache/images/``, creating parents as needed."""
    from mercury_constants import get_mercury_home

    path = get_mercury_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$MERCURY_HOME/cache/images/``.
