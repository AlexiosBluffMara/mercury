# Mercury — Nous Research Hermes Agent Creative Hackathon Submission

> **Mercury is a Hermes Agent fork that turns any short video into a 3D cortical
> activation map with three-tier narration, all on consumer hardware, using
> Kimi K2.6 + Hermes 4 405B + Gemma 4 in concert.**

- **Hackathon:** [Nous Research Hermes Agent Creative Hackathon](https://x.com/NousResearch/status/2039740198259462370) ($25K main + $5K Kimi Track)
...
2. **Hermes 4 405B** (Nous Portal) plans the tool sequence
...
of those — **video → 3D → long-form writing** — with the Hermes Agent
...
| **Creative software** | One Hermes Agent, four models, three platforms, *that the user can extend* — every chained capability is a skill in `skills/` |
...
(HermesHub, awesome-hermes-agent) had **almost nothing for 3D + neuroscience**
...
```
                     ┌──────────────────────────────────────────────┐
                     │  Mercury Hermes Agent (RTX 5090, Windows)    │
                     │                                              │
                     │  CLI ◄──┐    WebUI ◄─┐   gateway:            │
                     │         │            │     • Discord         │
                     │   ┌─────┴─────┐      │     • WhatsApp        │
...
- **Hermes 4 405B** plans the tool sequence (which skills, in what order,
...
```bash
# 1) Install Mercury (Hermes Agent fork)
...
|---|---|---|
| 0:00 | Title card | "Mercury — Hermes Agent + Cortex. RTX 5090, no cloud." |
...
| 2:30 | Multi-model swap | `/model nous-portal:nousresearch/hermes-4-405b` → re-narrate at researcher tier with Hermes 4 |
...
- [ ] **Public open-source GitHub repo** at `AlexiosBluffMara/mercury` with:
  - [x] MIT license inherited from upstream Hermes
...
1. **No `Co-Authored-By: Claude`** in commits. Match upstream Hermes
...
6. **`duration_trs` is hard-locked at 100 TRs (50 s @ 2 Hz).** Don't
   override.
7. **Kimi K2.6 is the default coder; Hermes 4 405B is the default planner.**
