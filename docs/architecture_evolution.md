...
A single Hermes Agent on the Chicago RTX 5090 with:
- **Two brains, role-split:** Hermes 4 405B as the planner, Kimi K2.6 as the coder.
...
1. **One brain.** Kimi K2.6 via Nous Portal handles both planning and code-gen in a single chain-of-tools loop. Hermes 4 405B is gone from the hot path. Cheaper, faster, less prompt-routing surface area.

2. **Skill auto-load by domain context.** Replaces the hard-coded skill list with the new Skill Dispatcher (loads matching `*.skill.md` from disk based on the user's prompt) plus the **Hermes Curator** (week-of-2026-04-26 launch — keeps skills lean, prunes/consolidates unused ones, skips pinned + bundled). New domains drop in without touching the agent loop.
...
4. **Six client surfaces, all wired.** Terminal, Discord, Web UI (Open WebUI compat), iMessage (BlueBubbles), Email (IMAP/SMTP), Mobile (Telegram + Signal). The Hermes Gateway routes them all to the same Tool Router → Skill Dispatcher.
...
The v2 picture tells the story of an **agent stack** that *contains* the brain visualizer (under `threejs-design-dev`) plus three other domains that have nothing to do with neuroscience but everything to do with what people actually use Hermes for: solving local problems with multi-tool agents (chicago-education, chicago-tax-legal) and packaging their own submissions (nous-hackathon).
