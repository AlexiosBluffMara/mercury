import type { RunExternalProcess } from '@mercury/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`mercury ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching mercury: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`mercury ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
