...
describe('shouldEmitClipboardSequence', () => {
  ...
  it('honors explicit env override', () => {
    expect(shouldEmitClipboardSequence({ MERCURY_TUI_CLIPBOARD_OSC52: '1', TMUX: '/tmp/tmux-1/default,1,0' } as NodeJS.ProcessEnv)).toBe(
      true
    )
    expect(shouldEmitClipboardSequence({ MERCURY_TUI_COPY_OSC52: '0', TERM: 'xterm-256color' } as NodeJS.ProcessEnv)).toBe(
      false
    )
  })
})
