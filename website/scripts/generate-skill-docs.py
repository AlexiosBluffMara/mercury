...
def render_skill_page(
    ...
    mercury_meta = (fm.get("metadata") or {}).get("mercury") or {}
    tags = mercury_meta.get("tags") or []
    related = mercury_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `mercury skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Mercury Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Mercury ships with a large built-in skill library copied into `~/.mercury/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
