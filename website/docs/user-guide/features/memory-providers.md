...
Mercury Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
mercury memory setup      # interactive picker + configuration
mercury memory status     # check what's active
mercury memory off        # disable external provider
```

You can also select the active memory provider via `mercury plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.mercury/config.yaml`:
...
When a memory provider is active, Mercury automatically:
...
**Setup Wizard:**
```bash
mercury honcho setup        # (legacy command) 
# or
mercury memory setup        # select "honcho"
```

**Config:** `$MERCURY_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$MERCURY_HOME/honcho.json` > `~/.mercury/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/mercury-ai/mercury-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/mercury).
...
:::tip Migrating from `mercury honcho`
If you previously used `mercury honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Mercury profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Mercury profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Mercury profile. Host key `mercury` → default; `mercury.<profile>` for others. |
...
```bash
mercury profile create coder --clone
```

`--clone` creates a `mercury.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Mercury profile, creates host blocks for any profile without one, inherits settings from the default `mercury` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/mercury-ai/mercury-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/mercury).
...
mercury memory setup    # select "openviking"
# Or manually:
mercury config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.mercury/.env
```
...
**Setup:**
```bash
mercury memory setup    # select "mem0"
# Or manually:
mercury config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.mercury/.env
```

**Config:** `$MERCURY_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `mercury-user` | User identifier |
| `agent_id` | `mercury` | Agent identifier |
...
**Setup:**
```bash
mercury memory setup    # select "hindsight"
# Or manually:
mercury config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.mercury/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p mercury ui start`

**Config:** `$MERCURY_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `mercury` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Mercury Agent and the User` | Context label for retained memories |
...
**Setup:**
```bash
mercury memory setup    # select "holographic"
# Or manually:
mercury config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.mercury-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$MERCURY_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
mercury memory setup    # select "retaindb"
# Or manually:
mercury config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.mercury/.env
```
...
# Then configure Mercury
mercury memory setup    # select "byterover"
# Or manually:
mercury config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$MERCURY_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
mercury memory setup    # select "supermemory"
# Or manually:
mercury config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.mercury/.env
```

**Config:** `$MERCURY_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `mercury` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `mercury-{identity}` → `mercury-coder`) to isolate memories per Mercury profile
...
- **Local storage providers** (Holographic, ByteRover) use `$MERCURY_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$MERCURY_HOME/` so each profile has its own credentials
