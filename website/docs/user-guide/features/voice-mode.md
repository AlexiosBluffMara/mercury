...
:::tip
The record key is configurable via `voice.record_key` in `~/.mercury/config.yaml` (default: `ctrl+b`).
:::
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.mercury/.env`
...
- Review gateway logs: `tail -f ~/.mercury/logs/gateway.log`
