...
Hermes reads MCP config from `~/.mercury/config.yaml` under `mcp_servers`.
...
The MCP server reads conversation data directly from Hermes's session store (`~/.mercury/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Hermes agent itself.
