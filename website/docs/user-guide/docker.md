...
sidebar_position: 7
title: "Docker"
description: "Running Mercury Agent in Docker and using Docker as a terminal backend"
---

# Mercury Agent — Docker

There are two distinct ways Docker intersects with Mercury Agent:

1. **Running Mercury IN Docker** — the agent itself runs inside a container (this page's primary focus)
...
If this is your first time running Mercury Agent, create a data directory on the host and start the container interactively to run the setup wizard:

```sh
mkdir -p ~/.mercury
docker run -it --rm \
  -v ~/.mercury:/opt/data \
  nousresearch/mercury-agent setup
```

This drops you into the setup wizard, which will prompt you for your API keys and write them to `~/.mercury/.env`. You only need to do this once. It is highly recommended to set up a chat system for the gateway to work with at this point.
...
docker run -d \
  ...
  -v ~/.mercury:/opt/data \
  -p 8642:8642 \
  nousresearch/mercury-agent gateway run
```
...
docker run -d \
  ...
  nousresearch/mercury-agent dashboard
```
...
docker run -it --rm \
  -v ~/.mercury:/opt/data \
  nousresearch/mercury-agent
```

Or if you have already opened a terminal in your running container (via Docker Desktop for instance), just run:

```sh
/opt/mercury/.venv/bin/mercury
```

## Persistent volumes

The `/opt/data` volume is the single source of truth for all Mercury state. It maps to your host's `~/.mercury/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All Mercury configuration |
...
:::warning
Never run two Mercury **gateway** containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent write access. Running a dashboard container alongside the gateway is safe since the dashboard only reads data.
:::
...
services:
  mercury:
    image: nousresearch/mercury-agent:latest
  ...
  dashboard:
    image: nousresearch/mercury-agent:latest
    container_name: mercury-dashboard
    restart: unless-stopped
    ...
    volumes:
      - ~/.mercury:/opt/data
    environment:
      - GATEWAY_HEALTH_URL=http://mercury:8642
    networks:
      - mercury-net
    depends_on:
      - mercury
...
The Mercury container needs moderate resources. Recommended minimums:
...
- Python 3 with all Mercury dependencies (`pip install -e ".[all]"`)
...
- Then runs `mercury` with whatever arguments you pass
...
```sh
docker pull nousresearch/mercury-agent:latest
docker rm -f mercury
docker run -d \
  --name mercury \
  --restart unless-stopped \
...
Check logs: `docker logs mercury`. Common causes:
- Missing or invalid `.env` file — run interactively first to complete setup
- Port conflicts if running with exposed ports

### "Permission denied" errors

The container runs as root by default. If your host `~/.mercury/` was created by a non-root user, permissions should work. If you get errors, ensure the data directory is writable:
...
```sh
docker restart mercury
```

### Checking container health

```sh
docker logs --tail 50 mercury          # Recent logs
docker run -it --rm nousresearch/mercury-agent:latest version     # Verify version
docker stats mercury                    # Resource usage
```
