...
Mercury Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.mercury/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.mercury/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`mercury` or `mercury chat`) |
...
mercury -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
mercury -c "my project"   # → resumes "my project #3"
```
...
mercury --resume 20250305_091523_a1b2c3d4
mercury -r 20250305_091523_a1b2c3d4

# Resume by title
mercury --resume "refactoring auth"

# Or with the chat subcommand
mercury chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `mercury sessions list`.

### Conversation Recap on Resume

When you resume a session, Mercury displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Mercury session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.mercury/config.yaml`:
...
Mercury automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `mercury sessions list` or `mercury sessions browse`.
...
```bash
mercury sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Mercury creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`mercury -c "my project"`), it automatically picks the most recent session in the lineage.
...
Mercury provides a full set of session management commands via `mercury sessions`:
...
# Filter by platform
mercury sessions list --source telegram

# Show more sessions
mercury sessions list --limit 50
```
...
mercury sessions export backup.jsonl

# Export sessions from a specific platform
mercury sessions export telegram-history.jsonl --source telegram

# Export a single session
mercury sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
mercury sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
mercury sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
mercury sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
mercury sessions prune

# Custom age threshold
mercury sessions prune --older-than 30

# Only prune sessions from a specific platform
mercury sessions prune --source telegram --older-than 60

# Skip confirmation
mercury sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`mercury insights`](/docs/reference/cli-commands#mercury-insights).
...
When Mercury cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Mercury uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Mercury in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.mercury/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.mercury/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.mercury/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Mercury process in the same `MERCURY_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.mercury/config.yaml`:
...
mercury sessions delete <session_id>
