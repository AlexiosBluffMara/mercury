...
title: "Git Worktrees"
description: "Run multiple Mercury agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Mercury Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Mercury so each session has a clean, isolated working directory.

## Why Use Worktrees with Mercury?

Mercury treats the **current working directory** as the project root:

- CLI: the directory where you run `mercury` or `mercury chat`
...
git worktree add ../repo-feature feature/mercury-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/mercury-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Mercury there:

```bash
cd ../repo-feature

# Start Mercury in the worktree
mercury
```
...
git worktree add ../repo-experiment-a feature/mercury-a
git worktree add ../repo-experiment-b feature/mercury-b
```
...
Each Mercury process:

- Works on its own branch (`feature/mercury-a` vs `feature/mercury-b`).
...
- Mercury checkpoint data under `~/.mercury/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Mercury experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/mercury-checkpoints-docs`, `feature/mercury-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Mercury from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `mercury -w` (Automatic Worktree Mode)

Mercury has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `mercury/mercury-<hash>`).
...
```bash
mercury -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `mercury -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Mercury session its own clean checkout.
