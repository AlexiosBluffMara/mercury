...
1. Enable via `mercury gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `mercury webhook subscribe`
...
Add the `github-pr` route to your `~/.mercury/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Mercury processes the event, and posts a review comment on the PR.
...
```bash
mercury webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `mercury webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
mercury webhook list
```

### Remove a subscription

```bash
mercury webhook remove github-issues
```

### Test a subscription

```bash
mercury webhook test github-issues
mercury webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.mercury/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `mercury webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `mercury gateway run`
