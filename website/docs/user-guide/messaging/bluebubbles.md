...
Connect Mercury to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Mercury and the BlueBubbles server
...
Or set environment variables directly in `~/.mercury/.env`:
...
When someone messages your iMessage, Mercury automatically sends them a pairing code. Approve it with:
```bash
mercury pairing approve bluebubbles <CODE>
```
Use `mercury pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.mercury/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.mercury/.env`):
```bash
...
Mercury will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Mercury
Mercury → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Mercury sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Mercury resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `mercury logs gateway` for webhook errors (or `mercury logs -f` to follow in real-time)
