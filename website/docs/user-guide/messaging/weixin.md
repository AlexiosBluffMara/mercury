...
5. Save the account credentials automatically to `~/.mercury/weixin/accounts/`
...
After initial QR login, set at minimum the account ID in `~/.mercury/.env`:
...
- Tokens are saved per account+peer to `~/.mercury/weixin/accounts/<account_id>.context-tokens.json`
