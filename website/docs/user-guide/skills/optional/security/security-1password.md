...
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Mercury Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Mercury terminal calls (desktop app flow only)
...
### Service Account (recommended for Mercury)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.mercury/.env` (the skill will prompt for this on first load).
...
## Mercury Execution Pattern (desktop app flow)

Mercury terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/mercury-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/mercury-op.sock"
