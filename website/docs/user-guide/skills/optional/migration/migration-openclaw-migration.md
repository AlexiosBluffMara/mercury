---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Mercury Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Mercury Agent"
---
...
Migrate a user's OpenClaw customization footprint into Mercury Agent. Imports Mercury-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Version | `1.0.0` |
| Author | Mercury Agent (Nous Research) |
| License | MIT |
| Tags | `Migration`, `OpenClaw`, `Mercury`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Mercury Migration

Use this skill when a user wants to move their OpenClaw setup into Mercury Agent with minimal manual cleanup.
...
```bash
mercury claw migrate              # Full interactive migration
mercury claw migrate --dry-run    # Preview what would be migrated
mercury claw migrate --preset user-data   # Migrate without secrets
mercury claw migrate --overwrite  # Overwrite existing conflicts
mercury claw migrate --source /custom/path/.openclaw  # Custom source
```
...
It uses `scripts/openclaw_to_mercury.py` to:

- import `SOUL.md` into the Mercury home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Mercury memory entries
- merge OpenClaw command approval patterns into Mercury `command_allowlist`
- migrate Mercury-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.mercury/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Mercury workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.mercury/tts/`
- archive non-secret docs that do not have a direct Mercury destination
...
- `scripts/openclaw_to_mercury.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.mercury/skills/migration/openclaw-migration/scripts/openclaw_to_mercury.py`

Do not guess a shorter path like `~/.mercury/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.mercury/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Mercury-compatible secrets, currently:
...
Mercury CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Mercury. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Mercury workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Mercury skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.mercury/skills/migration/openclaw-migration/scripts/openclaw_to_mercury.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.mercury/skills/migration/openclaw-migration/scripts/openclaw_to_mercury.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.mercury/skills/migration/openclaw-migration/scripts/openclaw_to_mercury.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.mercury/skills/migration/openclaw-migration/scripts/openclaw_to_mercury.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.mercury/skills/migration/openclaw-migration/scripts/openclaw_to_mercury.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.mercury/skills/migration/openclaw-migration/scripts/openclaw_to_mercury.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Mercury unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Mercury targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Mercury destination. Unsupported auth blobs must still be reported as skipped.
...
- Mercury persona state imported
- Mercury memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.mercury/skills/openclaw-imports/`
