...
| | |
|---|---|
| Source | Optional — install with `mercury skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Mercury or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Mercury. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.mercury/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.mercury/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Mercury integration, either:

- configure the server in `~/.mercury/config.yaml` using the `native-mcp` skill, or
...
### Mercury cannot see the deployed server

The server-building part may be correct while the Mercury config is not. Load the `native-mcp` skill and configure the server in `~/.mercury/config.yaml`, then restart Mercury.
