...
Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. mercury-agent@agentmail.to).

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `mercury skills install official/email/agentmail` |
...
Add to `~/.mercury/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `mercury-agent`)
   - The agent gets address: `mercury-agent@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "mercury-outreach")
...
```
mercury --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
