...
- Give it a name like "mercury-agent"
- Select scopes:
...
- Give it a title like "mercury-agent-&lt;machine-name>"
...
elif [ -f ~/.mercury/.env ] && grep -q "^GITHUB_TOKEN=" ~/.mercury/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.mercury/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
