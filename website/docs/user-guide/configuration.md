...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/hermes-agent` (HPC convention) → `~/.mercury/sandboxes/singularity`.
...
Hermes resolves each listed variable from your current shell first, then falls back to `~/.mercury/.env` if it was saved with `hermes config set`.
...
quick_commands:
  ...
  update:
    type: exec
    command: cd ~/.mercury/hermes-agent && git pull && pip install -e .
  gpu:
    type: exec
...
| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.mercury/SOUL.md` or `$HERMES_HOME/SOUL.md` |
