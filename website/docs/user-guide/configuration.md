...
sidebar_position: 2
title: "Configuration"
description: "Configure Mercury Agent — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.mercury/` directory for easy access.
...
```bash
mercury config              # View current configuration
mercury config edit         # Open config.yaml in your editor
mercury config set KEY VAL  # Set a specific value
mercury config check        # Check for missing options (after updates)
mercury config migrate      # Interactively add missing options

# Examples:
mercury config set model anthropic/claude-opus-4
mercury config set terminal.backend docker
mercury config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `mercury config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `mercury chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.mercury/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.mercury/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
You can set `providers.<id>.request_timeout_seconds` for a provider-wide request timeout, plus `providers.<id>.models.<model>.timeout_seconds` for a model-specific override. Applies to the primary turn client on every transport (OpenAI-wire, native Anthropic, Anthropic-compatible), the fallback chain, rebuilds after credential rotation, and (for OpenAI-wire) the per-request timeout kwarg — so the configured value wins over the legacy `MERCURY_API_TIMEOUT` env var.

You can also set `providers.<id>.stale_timeout_seconds` for the non-streaming stale-call detector, plus `providers.<id>.models.<model>.stale_timeout_seconds` for a model-specific override. This wins over the legacy `MERCURY_API_CALL_STALE_TIMEOUT` env var.
...
Mercury supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox, a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means Mercury will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
:::warning
The agent has the same filesystem access as your user account. Use `mercury tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Requirements:** Docker Desktop or Docker Engine installed and running. Mercury probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle).
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.mercury/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.mercury/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.mercury/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `mercury-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/mercury-agent` (HPC convention) → `~/.mercury/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `mercury config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Mercury logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `mercury doctor` to check.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.mercury/cache/documents:/output" # Gateway-visible exports
```
...
`/home/user/.mercury/cache/documents:/output`.

- Write files inside Docker to `/output/...`
- Emit the **host path** in `MEDIA:`, for example:
  `MEDIA:/home/user/.mercury/cache/documents/report.txt`
...
Mercury resolves each listed variable from your current shell first, then falls back to `~/.mercury/.env` if it was saved with `mercury config set`.
...
Docker sandboxes stay isolated by default. Mercury does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Mercury from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Mercury from
...
```bash
mercury config set terminal.persistent_shell false
```
...
- `mercury config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `mercury config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
mercury config set skills.config.myplugin.path ~/myplugin-data
```
...
Three related caps control how much raw output a tool can return before Mercury truncates it:
...
- **`max_bytes`** — When a `terminal` command produces more than this many characters of combined stdout/stderr, Mercury keeps the first 40% and last 60% and inserts a `[OUTPUT TRUNCATED]` notice between them. Default `50000` (≈12-15K tokens across typical tokenisers).
...
```yaml
worktree: true    # Always create a worktree (same as mercury -w)
...
Mercury automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `mercury plugins` → Provider Plugins → Context Engine.
...
Mercury has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.

| Timeout | Default | Local providers | Config / env |
|---------|---------|----------------|--------------|
| Socket read timeout | 120s | Auto-raised to 1800s | `MERCURY_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Auto-disabled | `MERCURY_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 300s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `MERCURY_API_CALL_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `providers.<id>.request_timeout_seconds` / `timeout_seconds` or `MERCURY_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Mercury raises this to 30 minutes when it detects a local endpoint. If you explicitly set `MERCURY_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Mercury disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `MERCURY_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
Mercury uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via auto-detection — you don't need to configure anything.

### The universal config pattern

Every model slot in Mercury — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Mercury ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Mercury uses that provider's built-in auth and base URL.
...
If you use a reasoning-heavy model for `auxiliary.session_search`, Mercury now gives you two built-in controls:

- `auxiliary.session_search.max_concurrency`: limits how many matched sessions Mercury summarizes at once
...
Use `extra_body` only when your provider documents OpenAI-compatible request-body fields you want Mercury to pass through for that task. Mercury forwards the object as-is.

:::warning
`extra_body` is only effective when your provider actually supports the field you send. If the provider does not expose a native OpenAI-compatible reasoning-off flag, Mercury cannot synthesize one on its behalf.
:::
...
Or via environment variable (in `~/.mercury/.env`):
...
| `"nous"` | Force Nous Portal | `mercury auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `mercury model` → Codex |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `mercury model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Mercury uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
`provider: "main"` uses whatever provider Mercury uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `mercury config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
`interim_assistant_messages` is gateway-only. When enabled, Mercury sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
If the requested provider is unavailable, Mercury falls back automatically in this order: `local` → `groq` → `openai`.
...
:::note
Streaming is disabled by default. Enable it in `~/.mercury/config.yaml` to try the streaming UX.
:::
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Mercury to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Mercury still keys DMs by chat/DM ID as usual.
...
Control what Mercury does when an unknown user sends a direct message:
...
- `pair` is the default. Mercury denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  status:
    type: exec
    command: systemctl status mercury-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.mercury/mercury-agent && git pull && pip install -e .
  gpu:
    type: exec
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (Mercury's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
The `web_search`, `web_extract`, and `web_crawl` tools support four backend providers. Configure the backend in `config.yaml` or via `mercury tools`:
...
**Exa:** Set `EXA_API_KEY` in `~/.mercury/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.mercury/browser_recordings/
  # Optional CDP override — when set, Mercury attaches directly to your own
...
Control how Mercury handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `MERCURY_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: true                  # Enable automatic checkpoints (also: mercury --checkpoints)
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Mercury falls back to `OPENAI_API_KEY` only.
...
Mercury uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.mercury/SOUL.md` or `$MERCURY_HOME/SOUL.md` |
| `.mercury.md` / `MERCURY.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, Mercury falls back to a built-in default identity.
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.mercury.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Mercury automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`mercury`)** | Current directory where you run the command |
| **Messaging gateway** | Home directory `~` (override with `MESSAGING_CWD`) |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```bash
# In ~/.mercury/.env or ~/.mercury/config.yaml:
