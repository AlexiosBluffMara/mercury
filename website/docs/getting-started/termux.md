...
description: "Run Mercury Agent directly on an Android phone with Termux"
---

# Mercury on Android with Termux

This is the tested path for running Mercury Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Mercury from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Mercury now ships a Termux-aware installer path:
...
- links `mercury` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `mercury` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/mercury" "$PREFIX/bin/mercury"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `mercury` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.mercury/.env`.
...
### `mercury doctor` says ripgrep or Node is missing
