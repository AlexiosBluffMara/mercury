...
The ACP adapter wraps Mercury' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
mercury acp / mercury-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.mercury/.env
  -> configure stderr logging
  -> construct MercuryACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Mercury `once`
- `allow_always` -> Mercury `always`
- reject options -> Mercury `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Mercury tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["mercury-acp"])
...
Instead it reuses Mercury' runtime resolver:

- `acp_adapter/auth.py`
- `mercury_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Mercury provider/credentials.
...
- `toolsets.py` — `mercury-acp` toolset definition
- `mercury_cli/main.py` — `mercury acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `mercury-acp` script
