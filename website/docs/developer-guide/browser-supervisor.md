...
(`mercury-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Mercury `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `mercury-acp`, `mercury-api-server`, core toolsets (gated on CDP reachability)
...
- `mercury_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
