...
description: "How Mercury builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Mercury deliberately separates:
...
1. agent identity — `SOUL.md` from `MERCURY_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.mercury/SOUL.md)
You are Mercury, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.mercury/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_mercury_md(cwd_path)       # 1. .mercury.md / MERCURY.md (walks to git root)
    ...
    # SOUL.md from MERCURY_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.mercury.md`, `MERCURY.md` | CWD up to git root | Mercury-native project config |
...
- **YAML frontmatter stripped** — `.mercury.md` frontmatter is removed (reserved for future config overrides)
...
1. `.mercury.md` / `MERCURY.md` (walks to git root)
