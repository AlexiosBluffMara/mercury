...
Jobs are stored in `~/.mercury/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
```python
# ~/.mercury/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.mercury/cron/output/` |
