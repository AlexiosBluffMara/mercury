...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Mercury-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Mercury Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Mercury/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- MercuryAgentBaseEnv
    MercuryAgentBaseEnv <|-- TerminalTestEnv
    MercuryAgentBaseEnv <|-- MercurySweEnv
    MercuryAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The mercury-agent layer (`environments/mercury_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls mercury-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `MercuryAgentLoop` and scores the result
...
Your environment inherits from `MercuryAgentBaseEnv` and implements five methods:
...
`MercuryAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as mercury-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any mercury-agent tool |
...
parser = get_parser("mercury")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `mercury`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "mercury-agent[yc-bench]"
...
```bash
python environments/mercury_swe_env/mercury_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"mercury"`, `"qwen"`, `"mistral"`)
...
```python
from environments.mercury_base_env import MercuryAgentBaseEnv, MercuryAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(MercuryAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(MercuryAgentBaseEnv):
    name = "my-env"
...
### MercuryAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which mercury toolsets to enable |
...
| `tool_call_parser` | `str` | `"mercury"` | Parser for Phase 2 raw output |
...
env:
  ...
  tokenizer_name: "NousResearch/Mercury-3-Llama-3.1-8B"
  use_wandb: true
...
- [Modal](https://modal.com) account and CLI: `pip install "mercury-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "mercury-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── mercury_base_env.py          # Abstract base class (MercuryAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (MercuryAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── mercury_parser.py        # Mercury/ChatML <tool_call> format
...
├── mercury_swe_env/             # SWE-bench training environment
│
