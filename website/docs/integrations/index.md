...
Mercury Agent connects to external systems for AI inference, tool servers, IDE workflows, programmatic access, and more. These integrations extend what Mercury can do and where it can run.

## AI Providers & Routing

Mercury supports multiple AI inference providers out of the box. Use `mercury model` to configure interactively, or set them in `config.yaml`.

- **[AI Providers](/docs/user-guide/features/provider-routing)** — OpenRouter, Anthropic, OpenAI, Google, and any OpenAI-compatible endpoint. Mercury auto-detects capabilities like vision, streaming, and tool use per provider.
...
- **[MCP Servers](/docs/user-guide/features/mcp)** — Connect Mercury to external tool servers via Model Context Protocol. Access tools from GitHub, databases, file systems, browser stacks, internal APIs, and more without writing native Mercury tools. Supports both stdio and SSE transports, per-server tool filtering, and capability-aware resource/prompt registration.

## Web Search Backends

The `web_search` and `web_extract` tools support four backend providers, configured via `config.yaml` or `mercury tools`:
...
Mercury includes full browser automation with multiple backend options for navigating websites, filling forms, and extracting information:
...
- **[IDE Integration (ACP)](/docs/user-guide/features/acp)** — Use Mercury Agent inside ACP-compatible editors such as VS Code, Zed, and JetBrains. Mercury runs as an ACP server, rendering chat messages, tool activity, file diffs, and terminal commands inside your editor.

## Programmatic Access

- **[API Server](/docs/user-guide/features/api-server)** — Expose Mercury as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox — can connect and use Mercury as a backend with its full toolset.
...
Mercury runs as a gateway bot on 15+ messaging platforms, all configured through the same `gateway` subsystem:
...
- **[Plugin System](/docs/user-guide/features/plugins)** — Extend Mercury with custom tools, lifecycle hooks, and CLI commands without modifying core code. Plugins are discovered from `~/.mercury/plugins/`, project-local `.mercury/plugins/`, and pip-installed entry points.
- **[Build a Plugin](/docs/guides/build-a-mercury-plugin)** — Step-by-step guide for creating Mercury plugins with tools, hooks, and CLI commands.
