---
sidebar_position: 7
title: "Use SOUL.md with Mercury"
description: "How to use SOUL.md to shape Mercury Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Mercury

`SOUL.md` is the **primary identity** for your Mercury instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Mercury to feel like the same assistant every time you talk to it — or if you want to replace the Mercury persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Mercury should be
- what Mercury should avoid stylistically
- how Mercury should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Mercury is and how Mercury speaks
...
Mercury now uses only the global SOUL file for the current instance:

```text
~/.mercury/SOUL.md
```

If you run Mercury with a custom home directory, it becomes:
...
Mercury automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Mercury does not overwrite it
- if the file exists but is empty, Mercury adds nothing from it to the prompt

## How Mercury uses it

When Mercury starts a session, it reads `SOUL.md` from `MERCURY_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Mercury feels.
...
Mercury already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Mercury should sound.

# Avoid
What Mercury should not do.

# Defaults
How Mercury should behave when ambiguity appears.
```
...
Then restart Mercury or start a new session.
...
### I edited SOUL.md but Mercury still sounds the same

Check:
- you edited `~/.mercury/SOUL.md` or `$MERCURY_HOME/SOUL.md`
...
### Mercury is ignoring parts of my SOUL.md
