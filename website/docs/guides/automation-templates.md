...
Copy-paste recipes for common automation patterns. Each template uses Mercury's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`mercury webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `mercury webhook subscribe`) |
...
```bash
mercury webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
mercury cron create "0 6 * * *" \
  "Run a dependency security audit on the mercury-agent project.

1. cd ~/.mercury/mercury-agent && source .venv/bin/activate
...
```bash
mercury webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
mercury webhook subscribe alert-triage \
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Mercury-Monitor/1.0"})
...
```bash
mercury cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Mercury Agent development." \
...
```bash
mercury webhook subscribe github-issues \
  --events "issues" \
...
```bash
mercury webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
mercury webhook subscribe stripe-payments \
...
```bash
mercury cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the mercury-agent codebase.
