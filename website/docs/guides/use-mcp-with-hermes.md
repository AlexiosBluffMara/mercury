---
sidebar_position: 6
title: "Use MCP with Mercury"
description: "A practical guide to connecting MCP servers to Mercury Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Mercury

This guide shows how to actually use MCP with Mercury Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Mercury tool
- you want Mercury to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Mercury to internal APIs, databases, or company systems without modifying Mercury core

Do not use MCP when:
- a built-in Mercury tool already solves the job well
...
- Mercury remains the agent
- MCP servers contribute tools
- Mercury discovers those tools at startup or reload time
...
If you installed Mercury with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Mercury banner/status should show MCP integration when configured
- ask Mercury what tools it has available
...
There are two categories of MCP-exposed functionality in Mercury:
...
2. Mercury-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Mercury will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Mercury to reason over a bounded workspace.
...
Now Mercury can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Mercury core.
...
Because Mercury now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
