---
sidebar_position: 8
title: "Use Voice Mode with Mercury"
description: "A practical guide to setting up and using Mercury voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Mercury
...
- you want Mercury sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Mercury.
...
## Step 1: make sure normal Mercury works first
...
Mercury supports both local and cloud speech stacks.
...
### If you use `mercury setup`

If you choose NeuTTS in the setup wizard, Mercury checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Mercury transcribes and responds
...
- asking Mercury to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Mercury loop.

## Tuning CLI behavior

### Silence threshold

If Mercury starts/stops too aggressively, tune:
...
Mercury stays a normal chat bot, but can speak replies.
...
- you want Mercury to function like a portable research or ops assistant
...
Mercury joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Mercury detects speech boundaries
- transcripts are posted in the associated text channel
- Mercury responds in text and audio
...
1. get text Mercury working
2. install `mercury-agent[voice]`
