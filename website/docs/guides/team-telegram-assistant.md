...
This tutorial walks you through setting up a Telegram bot powered by Mercury Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Mercury Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.mercury/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Mercury itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Mercury Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_mercury_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Mercury Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.mercury/.env`:
...
```
[Gateway] Starting Mercury Gateway...
...
# View live logs
journalctl --user -u mercury-gateway -f
...
mercury gateway start
mercury gateway stop
tail -f ~/.mercury/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `mercury gateway install` to update the plist.
:::
...
```bash
mercury gateway stop && mercury gateway start
```
...
mercury pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.mercury/.env`:
...
Control how much detail the bot shows when using tools. In `~/.mercury/config.yaml`:
...
Customize how the bot communicates by editing `~/.mercury/SOUL.md`:

For a full guide, see [Use SOUL.md with Mercury](/docs/guides/use-soul-with-mercury).
...
```bash
# From the CLI
mercury cron list          # View all scheduled jobs
mercury cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u mercury-gateway` (Linux) or `~/.mercury/logs/gateway.log` (macOS) |
| Cron job output | `~/.mercury/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.mercury/cron/jobs.json` |
| Pairing data | `~/.mercury/pairing/` |
| Session history | `~/.mercury/sessions/` |
