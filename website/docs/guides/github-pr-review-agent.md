...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Mercury Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Mercury instantly when PRs are opened or updated.
:::
...
- **Mercury Agent installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  mercury gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.mercury/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Mercury can access GitHub. Start a chat:
...
Still in the chat, ask Mercury to review a real PR:
...
A skill gives Mercury consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.mercury/skills/code-review
```

Create `~/.mercury/skills/code-review/SKILL.md`:
...
Verify it loaded — start `mercury` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Mercury your team's standards:
...
2. Teach Mercury your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
mercury gateway status    # Is the gateway running?
mercury cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-mercury-plugin)** — wrap the review logic into a shareable plugin
