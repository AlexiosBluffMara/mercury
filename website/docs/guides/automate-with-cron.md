...
description: "Real-world automation patterns using Mercury cron — monitoring, reports, pipelines, and multi-skill workflows"
---
...
```bash
mkdir -p ~/.mercury/scripts
```

```python title="~/.mercury/scripts/watch-site.py"
import hashlib, json, os, urllib.request

URL = "https://example.com/pricing"
STATE_FILE = os.path.expanduser("~/.mercury/scripts/.watch-site-state.json")

# Fetch current content
req = urllib.request.Request(URL, headers={"User-Agent": "Mercury-Monitor/1.0"})
...
```bash
/cron add "every 1h" "If the script output says CHANGE DETECTED, summarize what changed on the page and why it might matter. If it says NO_CHANGE, respond with just [SILENT]." --script ~/.mercury/scripts/watch-site.py --name "Pricing monitor" --deliver telegram
```
...
```python title="~/.mercury/scripts/collect-prices.py"
import json, os, urllib.request
from datetime import datetime

DATA_DIR = os.path.expanduser("~/.mercury/data/prices")
...
If there's a significant move, explain what happened." \
  --script ~/.mercury/scripts/collect-prices.py \
  --name "Price tracker" \
  --deliver telegram
```
