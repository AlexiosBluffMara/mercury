...
Mercury has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `mercury_cli/commands.py`:
...
Installed skills are also exposed as dynamic slash commands on both surfaces. That includes bundled skills like `/plan`, which opens plan mode and saves markdown plans under `.mercury/plans/` relative to the active workspace/backend working directory.
...
| `/snapshot [create\|restore <id>\|prune]` (alias: `/snap`) | Create or restore state snapshots of Mercury config/state. `create [label]` saves a snapshot, `restore <id>` reverts to it, `prune [N]` removes old snapshots, or list all with no args. |
...
User-defined quick commands map a short alias to a longer prompt. Configure them in `~/.mercury/config.yaml`:
