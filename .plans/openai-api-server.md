# OpenAI-Compatible API Server for Mercury Agent
...
SSE streaming. By exposing this endpoint, mercury-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  mercury-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.mercury/.env`
2. Run `mercury gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with mercury-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns mercury-agent as a model) |
...
Authorization: Bearer mercury-api-key-here
...
But mercury-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "mercury-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured mercury-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of mercury-agent's core prompt
...
Once implemented, mercury-agent works as a drop-in backend for:
