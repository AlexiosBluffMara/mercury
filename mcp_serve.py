"""
Mercury MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Mercury-specific extra)

Usage:
    mercury mcp serve
    mercury mcp serve --verbose
...
logger = logging.getLogger("mercury.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using MERCURY_HOME."""
    try:
        from mercury_constants import get_mercury_home
        return get_mercury_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("MERCURY_HOME", Path.home() / ".mercury")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from mercury_constants import get_mercury_home
        directory_file = get_mercury_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("MERCURY_HOME", Path.home() / ".mercury")
...
class EventBridge:
    ...
    This is the Mercury equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from mercury_constants import get_mercury_home
            db_file = get_mercury_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("MERCURY_HOME", Path.home() / ".mercury")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Mercury MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "mercury",
        instructions=(
            "Mercury Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Mercury MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
