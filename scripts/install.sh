...
if [ -n "${MERCURY_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$MERCURY_INSTALL_DIR"
...
#   code at /usr/local/lib/mercury-agent, command at /usr/local/bin/mercury,
#   data still at /root/.mercury (MERCURY_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.mercury/mercury-agent"
            echo "                   default (root, Linux): /usr/local/lib/mercury-agent"
            echo "  --mercury-home PATH  Data directory (default: ~/.mercury, or \$MERCURY_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Mercury installs the code under"
            echo "  /usr/local/lib/mercury-agent and links the command into"
            echo "  /usr/local/bin/mercury (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$MERCURY_HOME"
            echo "  (default /root/.mercury).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$MERCURY_HOME/mercury-agent are preserved in-place."
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `mercury` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $MERCURY_HOME/mercury-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $MERCURY_HOME/mercury-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/mercury-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $MERCURY_HOME/mercury-agent — then preserve it)
#
# Always no-op when the user set --dir or $MERCURY_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in MERCURY_HOME.
    if is_termux; then
        INSTALL_DIR="$MERCURY_HOME/mercury-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$MERCURY_HOME/mercury-agent/.git" ]; then
            INSTALL_DIR="$MERCURY_HOME/mercury-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/mercury-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/mercury-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/mercury"
        log_info "  Data:    $MERCURY_HOME (unchanged)"
        return 0
    fi
...
get_mercury_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/mercury" ]; then
        echo "$link_dir/mercury"
    else
        echo "mercury"
    fi
}
...
check_node() {
    ...
    if [ -x "$MERCURY_HOME/node/bin/node" ]; then
        export PATH="$MERCURY_HOME/node/bin:$PATH"
        local found_ver=$("$MERCURY_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Mercury-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.mercury/node/..."
    ...
    # Place into ~/.mercury/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$MERCURY_HOME/node"
    mkdir -p "$MERCURY_HOME"
    mv "$extracted_dir" "$MERCURY_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$MERCURY_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$MERCURY_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$MERCURY_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$MERCURY_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$MERCURY_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.mercury/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Mercury Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="mercury-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Mercury behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up mercury command..."

    if [ "$USE_VENV" = true ]; then
        MERCURY_BIN="$INSTALL_DIR/venv/bin/mercury"
    else
        MERCURY_BIN="$(which mercury 2>/dev/null || echo "")"
        if [ -z "$MERCURY_BIN" ]; then
            log_warn "mercury not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$MERCURY_BIN" ]; then
        log_warn "mercury entry point not found at $MERCURY_BIN"
    ...
    # Create a user-facing shim for the mercury command.
    mkdir -p "$command_link_dir"
    ln -sf "$MERCURY_BIN" "$command_link_dir/mercury"
    log_success "Symlinked mercury → $command_link_display_dir/mercury"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "mercury command ready"
        return 0
    fi
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Mercury Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Mercury Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so mercury works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.mercury directory structure (config at top level, code in subdir)
    mkdir -p "$MERCURY_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.mercury/.env (top level, easy to find)
    if [ ! -f "$MERCURY_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$MERCURY_HOME/.env"
            log_success "Created ~/.mercury/.env from template"
        else
            touch "$MERCURY_HOME/.env"
            log_success "Created ~/.mercury/.env"
        fi
    else
        log_info "~/.mercury/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.mercury/config.yaml (top level, easy to find)
    if [ ! -f "$MERCURY_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$MERCURY_HOME/config.yaml"
            log_success "Created ~/.mercury/config.yaml from template"
        fi
    else
        log_info "~/.mercury/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$MERCURY_HOME/SOUL.md" ]; then
        cat > "$MERCURY_HOME/SOUL.md" << 'SOUL_EOF'
# Mercury Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Mercury communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.mercury/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.mercury/"

    # Seed bundled skills into ~/.mercury/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.mercury/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.mercury/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$MERCURY_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$MERCURY_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.mercury/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Mercury itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (mercury --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'mercury setup' after install."
        return 0
    fi
    ...
    # Run mercury setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m mercury_cli.main setup < /dev/tty
    else
        python -m mercury_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$MERCURY_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Mercury to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$MERCURY_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'mercury whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                MERCURY_CMD="$(get_mercury_command_path)"
                $MERCURY_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'mercury whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'mercury gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $MERCURY_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $MERCURY_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: mercury gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: mercury gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.mercury/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: mercury gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: mercury gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $MERCURY_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $MERCURY_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $MERCURY_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}mercury${NC}              Start chatting"
    echo -e "   ${GREEN}mercury setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}mercury config${NC}       View/edit configuration"
    echo -e "   ${GREEN}mercury config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}mercury gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}mercury update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'mercury' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'mercury' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'mercury' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
