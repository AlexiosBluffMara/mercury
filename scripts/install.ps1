# ============================================================================
# Mercury Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$MercuryHome = "$env:LOCALAPPDATA\mercury",
    [string]$InstallDir = "$env:LOCALAPPDATA\mercury\mercury-agent"
)
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Mercury Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Test-Node {
    ...
    $managedNode = "$MercuryHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$MercuryHome\node;$env:Path"
        Write-Success "Node.js $version found (Mercury-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.mercury/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\mercury-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$MercuryHome\node") { Remove-Item -Recurse -Force "$MercuryHome\node" }
                Move-Item $extractedDir.FullName "$MercuryHome\node"
                $env:Path = "$MercuryHome\node;$env:Path"

                $version = & "$MercuryHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.mercury/node/"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $zipPath = "$env:TEMP\mercury-agent-$Branch.zip"
                $extractPath = "$env:TEMP\mercury-agent-extract"
...
function Set-PathVariable {
    Write-Info "Setting up mercury command..."
    
    if ($NoVenv) {
        $mercuryBin = "$InstallDir"
    } else {
        $mercuryBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so mercury is globally available
    # On Windows, the mercury.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$mercuryBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$mercuryBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $mercuryBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set MERCURY_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\mercury instead
    # of the Unix default ~/.mercury
    $currentMercuryHome = [Environment]::GetEnvironmentVariable("MERCURY_HOME", "User")
    if (-not $currentMercuryHome -or $currentMercuryHome -ne $MercuryHome) {
        [Environment]::SetEnvironmentVariable("MERCURY_HOME", $MercuryHome, "User")
        Write-Success "Set MERCURY_HOME=$MercuryHome"
    }
    $env:MERCURY_HOME = $MercuryHome
    
    # Update current session
    $env:Path = "$mercuryBin;$env:Path"
    
    Write-Success "mercury command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.mercury directory structure
    New-Item -ItemType Directory -Force -Path "$MercuryHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MercuryHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MercuryHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MercuryHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MercuryHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MercuryHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MercuryHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MercuryHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$MercuryHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$MercuryHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.mercury/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.mercury/.env"
        }
    } else {
        Write-Info "~/.mercury/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$MercuryHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.mercury/config.yaml from template"
        }
    } else {
        Write-Info "~/.mercury/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$MercuryHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.mercury/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.mercury/"
    
    # Seed bundled skills into ~/.mercury/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.mercury/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.mercury/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$MercuryHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.mercury/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        try {
            ...
            Write-Warn "TUI npm install failed (mercury --tui may not work)"
        }
        Pop-Location
    }
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m mercury_cli.main setup
    } else {
        python -m mercury_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $mercuryCmd = "$InstallDir\venv\Scripts\mercury.exe"
    if (-not (Test-Path $mercuryCmd)) {
        $mercuryCmd = "mercury"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$MercuryHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'mercury whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$MercuryHome\logs\gateway.log"
            Start-Process -FilePath $mercuryCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$MercuryHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: mercury gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: mercury gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$MercuryHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$MercuryHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MercuryHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$MercuryHome\mercury-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   mercury              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   mercury setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   mercury config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   mercury config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   mercury gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   mercury update       " -NoNewline -ForegroundColor Green
