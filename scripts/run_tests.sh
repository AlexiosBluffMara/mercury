#!/usr/bin/env bash
# Canonical test runner for mercury-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.mercury/mercury-agent/venv"; do
...
# Unset MERCURY_* behavioral vars too.
unset MERCURY_YOLO_MODE MERCURY_INTERACTIVE MERCURY_QUIET MERCURY_TOOL_PROGRESS \
      MERCURY_TOOL_PROGRESS_MODE MERCURY_MAX_ITERATIONS MERCURY_SESSION_PLATFORM \
      MERCURY_SESSION_CHAT_ID MERCURY_SESSION_CHAT_NAME MERCURY_SESSION_THREAD_ID \
      MERCURY_SESSION_SOURCE MERCURY_SESSION_KEY MERCURY_GATEWAY_SESSION \
      MERCURY_PLATFORM MERCURY_INFERENCE_PROVIDER MERCURY_MANAGED MERCURY_DEV \
      MERCURY_CONTAINER MERCURY_EPHEMERAL_SYSTEM_PROMPT MERCURY_TIMEZONE \
      MERCURY_REDACT_SECRETS MERCURY_BACKGROUND_NOTIFICATIONS MERCURY_EXEC_ASK \
      MERCURY_HOME_MODE 2>/dev/null || true
...
WORKERS="${MERCURY_TEST_WORKERS:-4}"
