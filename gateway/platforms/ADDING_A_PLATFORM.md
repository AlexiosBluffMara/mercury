...
Checklist for integrating a new messaging platform into the Mercury gateway.
...
And add it to the `mercury-gateway` composite:

```python
"mercury-gateway": {
    "includes": [..., "mercury-your-platform"]
}
```
...
## 12. Status Display (`mercury_cli/status.py`)
...
## 13. Gateway Setup Wizard (`mercury_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ mercury_cli/ toolsets.py \
  --include="*.py" -l | sort -u
