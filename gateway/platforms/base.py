...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.mercury/.env manually."
)
...
# Default location: {MERCURY_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_mercury_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; MercuryAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
AUDIO_CACHE_DIR = get_mercury_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_mercury_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_mercury_dir("cache/documents", "document_cache")
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from mercury_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          MERCURY_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          MERCURY_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          MERCURY_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("MERCURY_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        min_ms = int(os.getenv("MERCURY_HUMAN_DELAY_MIN_MS", "800"))
        max_ms = int(os.getenv("MERCURY_HUMAN_DELAY_MAX_MS", "2500"))
        if mode == "natural":
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        callback_generation = getattr(interrupt_event, "_mercury_run_generation", None)
