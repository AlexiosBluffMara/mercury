...
Auto-tracks and cleans up ephemeral files created during Mercury Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$MERCURY_HOME` and `/tmp/mercury-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `MERCURY_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `MERCURY_HOME` or `/tmp/mercury-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$MERCURY_HOME/disk-cleanup/` is itself excluded
- `$MERCURY_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
