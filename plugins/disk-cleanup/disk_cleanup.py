"""disk_cleanup — ephemeral file cleanup for Mercury Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under MERCURY_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly MERCURY_HOME and /tmp/mercury-*
Never touches: ~/.mercury/logs/ or any system directory.
"""
...
try:
    ...
    def get_mercury_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("MERCURY_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".mercury").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$MERCURY_HOME/logs/``."""
    return get_mercury_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$MERCURY_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under MERCURY_HOME or ``/tmp/mercury-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    mercury_home = get_mercury_home()
    try:
        path.resolve().relative_to(mercury_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/mercury-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("mercury-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside MERCURY_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under MERCURY_HOME (but leave MERCURY_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "mercury-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(mercury_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == mercury_home:
                continue
            try:
                rel_parts = dirpath.relative_to(mercury_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    mercury_home = get_mercury_home()
    try:
        rel = path.resolve().relative_to(mercury_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "mercury-agent",
        }:
            return None
        ...
        # Path isn't under MERCURY_HOME (e.g. /tmp/mercury-*) — fall through.
        pass
