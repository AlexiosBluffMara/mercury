"""Persistent slash-command worker — one MercuryCLI per TUI session.
...
def _run(cli: MercuryCLI, command: str) -> str:
...
def main():
    ...
    os.environ["MERCURY_SESSION_KEY"] = args.session_key
    os.environ["MERCURY_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = MercuryCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
