"""
Interactive setup wizard for Mercury Agent.
...
from mercury_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://mercury-agent.nousresearch.com/docs"
...
# display_mercury_home imported lazily at call sites (stale-module safety during mercury update)
...
from mercury_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Mercury Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Mercury using environment variables or config commands:")
    print_info("  mercury config set model.provider custom")
    print_info("  mercury config set model.base_url http://localhost:8080/v1")
    print_info("  mercury config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'mercury setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'mercury setup')")


def _print_setup_summary(config: dict, mercury_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'mercury setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'mercury setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'mercury setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'mercury setup terminal'"))
    ...
    # Spotify (OAuth via mercury auth spotify — check auth.json, not env vars)
    try:
        from mercury_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'mercury setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('mercury setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('mercury setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('mercury setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('mercury setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('mercury setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('mercury config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('mercury config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('mercury config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('mercury', Colors.GREEN)}              Start chatting")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``mercury model``)
    ...
    provider added to ``mercury model`` is automatically available here.
    ...
    # Delegate to the shared mercury model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from mercury_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: mercury model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from mercury_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Mercury can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Mercury can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'mercury setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.mercury/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"mercury setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Mercury runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Mercury via Telegram/Discord, this is where")
        print_info(
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("MERCURY_MAX_ITERATIONS", "90")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("MERCURY_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("MERCURY_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Mercury delivers cron job results,")
...
def _setup_slack():
    ...
    print()
    print_info("   Full guide: https://mercury-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Mercury delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Mercury delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'mercury whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'mercury whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from mercury_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from mercury_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from mercury_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from mercury_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from mercury_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from mercury_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from mercury_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from mercury_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_bluebubbles():
    ...
    print_info("Connects Mercury to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from mercury_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://mercury-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://mercury-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  mercury config edit")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     mercury config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_mercury_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  mercury gateway")
        else:
            from mercury_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   mercury gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   mercury gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `mercury setup tools` and `mercury tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``mercury_cli.auth`` — lists every supported
    ...
    """
    try:
        from mercury_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from mercury_cli.config
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_mercury.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_mercury migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_mercury", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Mercury.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Mercury to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Mercury at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Mercury at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Mercury at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Mercury at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Mercury equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Mercury config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Mercury.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Mercury's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(mercury_home: Path) -> bool:
    ...
    print_info("Mercury can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: mercury claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: mercury claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Mercury configs are
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Mercury (use mercury claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Mercury Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Mercury Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Mercury Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Mercury configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, mercury_home)
            return
        elif choice == 1:
            # Full setup — fall through to run all sections
            pass
        elif choice == 7:
            print_info("Exiting. Run 'mercury setup' again when ready.")
            return
        elif 2 <= choice <= 6:
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, mercury_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(mercury_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Mercury?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, mercury_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {mercury_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'mercury config edit'")
...
def _resolve_mercury_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``mercury chat`` in a fresh process."""
    mercury_bin = shutil.which("mercury")
    if mercury_bin:
        return [mercury_bin, "chat"]

    try:
        if importlib.util.find_spec("mercury_cli") is not None:
            return [sys.executable, "-m", "mercury_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch mercury chat now?", True):
        return

    chat_argv = _resolve_mercury_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch Mercury automatically. Run 'mercury chat' manually.")
        return

    os.execvp(chat_argv[0], chat_argv)


def _run_first_time_quick_setup(config: dict, mercury_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'mercury setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    mercury setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  mercury setup gateway")
    print()
...
def _run_quick_setup(config: dict, mercury_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'mercury setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Mercury to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'mercury setup gateway'.")
