"""Shell completion script generation for mercury CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_mercury_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Mercury Agent bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef mercury
# Mercury Agent zsh completion
# Add to ~/.zshrc:
...
_mercury() {{
    ...
    _arguments -C \\
        ...
        '(-p --profile){{-p,--profile}}[Profile name]:profile:_mercury_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'mercury command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Mercury Agent fish completion",
        "# Add to your config:",
        "#   mercury completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __mercury_profiles",
        "    echo default",
        "    if test -d $HOME/.mercury/profiles",
        "        ls $HOME/.mercury/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c mercury -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c mercury -f -s p -l profile"
        " -d 'Profile name' -xa '(__mercury_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__mercury_profiles)' -d 'Profile name'"
                )
