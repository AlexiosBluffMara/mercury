"""
Gateway subcommand for mercury CLI.

Handles: mercury gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_mercury_home is imported lazily at call sites to avoid ImportError
# when mercury_constants is cached from a pre-update version during `mercury update`.
from mercury_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from mercury_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "mercury-gateway*",
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    patterns = [
        ...
        "mercury_cli.main --profile",
        "mercury_cli.main -p",
        "mercury_cli/main.py gateway",
        "mercury_cli/main.py --profile",
        "mercury_cli/main.py -p",
        "mercury gateway",
        "gateway/run.py",
    ]
    current_home = str(get_mercury_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"MERCURY_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "MERCURY_HOME=" in command and f"MERCURY_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``mercury update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Mercury profile are returned.
    """
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}mercury gateway status\n"
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from mercury_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `mercury gateway`")
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (MERCURY_HOME-scoped).
...
_SERVICE_BASE = "mercury-gateway"
SERVICE_DESCRIPTION = "Mercury Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current MERCURY_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (MERCURY_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from mercury_constants import get_default_mercury_root
    home = get_mercury_home().resolve()
    default = get_default_mercury_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary MERCURY_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(mercury_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when MERCURY_HOME is a named profile.
    ...
    Args:
        mercury_home: Optional explicit MERCURY_HOME path. Defaults to the current
            ``get_mercury_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from mercury_constants import get_default_mercury_root
    home = Path(mercury_home or str(get_mercury_home())).resolve()
    default = get_default_mercury_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
# Legacy service names from older Mercury installs that predate the
# mercury-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (mercury-gateway-*.service) and unrelated third-party
# "mercury" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("mercury.service",)
...
def _find_legacy_mercury_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Mercury gateway units.

    Detects unit files installed by older Mercury versions that used a
    different service name (e.g. ``mercury.service`` before the rename to
    ``mercury-gateway.service``). When both a legacy unit and the current
    ``mercury-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``mercury-gateway-coder.service`` and unrelated third-party
      ``mercury-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``mercury.service`` running an unrelated
...
def has_legacy_mercury_units() -> bool:
    """Return True when any legacy Mercury gateway unit files exist."""
    return bool(_find_legacy_mercury_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Mercury gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_mercury_units()
    if not legacy:
        return
    print_warning("Legacy Mercury gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current mercury-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    mercury gateway migrate-legacy")


def remove_legacy_mercury_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Mercury gateway unit files.

    Iterates over whatever ``_find_legacy_mercury_units()`` returns — which is
    ...
    legacy = _find_legacy_mercury_units()
    if not legacy:
        print("No legacy Mercury gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Mercury gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: mercury gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo mercury gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    mercury gateway uninstall")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Mercury can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo mercury gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo mercury gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo mercury gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Mercury often sets ``HOME`` to a profile-scoped directory, but
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.mercury/mercury-agent  -> /home/alice/.mercury/mercury-agent
      /opt/mercury                 -> /opt/mercury  (kept as-is)
...
def _mercury_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current MERCURY_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_mercury_home() resolves to
    ...
    """
    current_mercury = get_mercury_home().resolve()
    current_default = (Path.home() / ".mercury").resolve()
    target_default = Path(target_home_dir) / ".mercury"

    # Default ~/.mercury → remap to target user's default
    if current_mercury == current_default:
        return str(target_default)

    # Profile or subdir of ~/.mercury → preserve the relative structure
    try:
        relative = current_mercury.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.mercury) — keep as-is
        return str(current_mercury)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        mercury_home = _mercury_home_for_target_user(home_dir)
        profile_arg = _profile_arg(mercury_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m mercury_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
"""
    ...
    mercury_home = str(get_mercury_home().resolve())
    profile_arg = _profile_arg(mercury_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Mercury install")
    return True
...
def _get_restart_drain_timeout() -> float:
    ...
    raw = os.getenv("MERCURY_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (mercury.service from pre-rename installs)
    # before installing the new mercury-gateway.service. If both remain, they
    ...
    if has_legacy_mercury_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_mercury_units(interactive=False)
            print()
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}mercury gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}mercury gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `mercury gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}mercury gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}mercury gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}mercury gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}mercury gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.mercury.gateway-{suffix}" if suffix else "ai.mercury.gateway"
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>mercury_cli.main</string>",
    ]
    if profile_arg:
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Mercury install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  mercury gateway status             # Check status")
    from mercury_constants import display_mercury_home as _dhh
...
def launchd_stop():
    ...
    # `mercury gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Mercury install")
    else:
        print("⚠ Service definition is stale relative to the current Mercury install")
        print("  Run: mercury gateway start")
    ...
    if deep:
        log_file = get_mercury_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Mercury Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Mercury will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @mercury:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. mercury) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Mercury delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Mercury agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Mercury will use (e.g., mercury@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Mercury connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: mercury pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_mercury_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'mercury pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: mercury pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'mercury gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from mercury_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from mercury_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Mercury will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Mercury will store the returned account_id/token in ~/.mercury/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `mercury gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_mercury_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `mercury pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `mercury pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"MercuryAgent\"")
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: mercury gateway install")
                else:
                    print_info("  You can install later: mercury gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo mercury gateway install --system")
                    print_info("  Or run in foreground:  mercury gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: mercury gateway run")
                print_info("  For persistence:   tmux new -s mercury 'mercury gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: mercury gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup mercury gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: mercury gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'mercury gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: mercury gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: mercury gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s mercury 'mercury gateway run'")
                print()
            ...
            print()
            print("  mercury gateway run                              # direct foreground")
            print("  tmux new -s mercury 'mercury gateway run'         # persistent via tmux")
            print("  nohup mercury gateway run > ~/.mercury/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: mercury gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: mercury gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: mercury gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: mercury gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: mercury gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  mercury gateway install")
                    print("  sudo mercury gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  mercury gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup mercury gateway run > ~/.mercury/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s mercury 'mercury gateway run'         # persistent via tmux")
                    print("  nohup mercury gateway run > ~/.mercury/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  mercury gateway install  # Install as user service")
                    print("  sudo mercury gateway install --system  # Install as boot-time system service")

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy Mercury gateway unit files from
        # pre-rename installs (e.g. mercury.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_mercury_units(interactive=not yes, dry_run=dry_run)
