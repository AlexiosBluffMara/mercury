...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or mercury -c.",
    ...
    "Ctrl+Z suspends Mercury to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "mercury -c resumes your most recent CLI session. mercury -c \"project name\" resumes by title.",
    "mercury -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "mercury -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "mercury chat -t web,terminal enables only specific toolsets for a focused session.",
    "mercury chat -s github-pr-workflow preloads a skill at launch.",
    "mercury chat -q \"query\" runs a single non-interactive query and exits.",
    "mercury chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "mercury chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "mercury --yolo bypasses all dangerous command approval prompts for the entire session.",
    "mercury chat --source telegram tags the session for filtering in mercury sessions list.",
    "mercury -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "mercury doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "mercury dump outputs a compact setup summary — great for bug reports.",
    "mercury config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "mercury config edit opens config.yaml in your default editor.",
    "mercury config check scans for missing or stale configuration options.",
    "mercury sessions browse opens an interactive session picker with search.",
    "mercury sessions stats shows session counts by platform and database size.",
    "mercury sessions prune --older-than 30 cleans up old sessions.",
    "mercury skills search react --source skills-sh searches the skills.sh public directory.",
    "mercury skills check scans installed hub skills for upstream updates.",
    "mercury skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "mercury skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "mercury mcp add github --command npx adds MCP servers from the command line.",
    "mercury mcp serve runs Mercury itself as an MCP server for other agents.",
    "mercury auth add lets you add multiple API keys for credential pool rotation.",
    "mercury completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "mercury logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "mercury backup creates a zip backup of your entire Mercury home directory.",
    "mercury profile create coder creates an isolated profile that becomes its own command.",
    "mercury profile create work --clone copies your current config and keys to a new profile.",
    "mercury update syncs new bundled skills to ALL profiles automatically.",
    "mercury gateway install sets up Mercury as a system service (systemd/launchd).",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as mercury -w).",
    ...
    "MERCURY_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Mercury tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'mercury profile create coder' creates the 'coder' command.",
    "mercury profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Mercury prints a resume command with session ID and stats.",
    "mercury sessions export backup.jsonl exports all sessions for backup or analysis.",
    "mercury -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "mercury skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .mercury/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: mercury cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Mercury runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "mercury gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Mercury via a local macOS server.",
    ...
    "Context files (.mercury.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.mercury/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Mercury loads project context from .mercury.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — mercury tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.mercury/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "mercury chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "mercury plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "mercury acp runs Mercury as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "MERCURY_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "mercury login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "mercury status --deep runs deeper diagnostic checks across all components.",

    # --- Hidden Gems & Power-User Tricks ---
    "BOOT.md at ~/.mercury/BOOT.md runs automatically on every gateway start — use it for startup checks.",
    "Cron jobs can attach a Python script (--script) whose stdout is injected into the prompt as context.",
    "Cron scripts live in ~/.mercury/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Mercury your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.mercury/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Mercury auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.mercury/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at MERCURY_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "MERCURY_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in MERCURY_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.mercury/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'mercury profile create ls' would be rejected.",
    "mercury profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "mercury claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "MERCURY_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via mercury tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.mercury/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.mercury/skills/.hub/quarantine/ holds skills pending security review.",
]
