"""mercury hooks — inspect and manage shell-script hooks.

Usage::

    mercury hooks list
    mercury hooks test <event> [--for-tool X] [--payload-file F]
    mercury hooks revoke <command>
    mercury hooks doctor

Consent records live under ``~/.mercury/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.mercury/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``mercury hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: mercury hooks {list|test|revoke|doctor}")
        print("Run 'mercury hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.mercury/config.yaml.")
        print("See `mercury hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `mercury hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `mercury hooks test` and `mercury hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Mercury wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `mercury hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `mercury hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `mercury hooks doctor`.")
