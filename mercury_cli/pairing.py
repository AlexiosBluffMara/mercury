...
Usage:
    mercury pairing list              # Show all pending + approved users
    mercury pairing approve <platform> <code>  # Approve a pairing code
    mercury pairing revoke <platform> <user_id> # Revoke user access
    mercury pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle mercury pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: mercury pairing {list|approve|revoke|clear-pending}")
        print("Run 'mercury pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print("  Run 'mercury pairing list' to see pending codes.\n")
