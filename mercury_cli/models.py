...
Add, remove, or reorder entries here — both `mercury setup` and
`mercury` provider-selection will pick up the change automatically.
"""
...
_MERCURY_USER_AGENT = f"mercury-cli/{_MERCURY_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `mercury model`
    flow without maintaining a separate static list.
    """
    from mercury_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from mercury_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  mercury model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `mercury model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `mercury model` TUI
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry("evolink",        "EvoLink",                  "EvoLink (OpenAI-compatible unified model API)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``mercury model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``mercury auth add openai-codex`` without
    ``mercury model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    mercury-agent is tool-calling-first — every provider path assumes the model
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — mercury-agent
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from mercury_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return mercury-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; mercury's picker expects
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``mercury model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from mercury_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Mercury' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``mercury_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Mercury should expose the /fast toggle for this model."""
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Mercury release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "nous":
        ...
        try:
            from mercury_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from mercury_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "MercuryAgent/1.0",
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Mercury' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _MERCURY_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_mercury_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Mercury will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Mercury cannot verify the model name."
