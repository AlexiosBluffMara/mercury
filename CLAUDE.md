...
| **Planner** | Mercury 4 405B via Nous Portal | Agentic planning loops inside Mercury |
...
python tools/kimi_dispatch.py --task path/to/spec.md --planner   # Mercury 4 405B
...
5. **No `Co-Authored-By: Claude`** in commits. Match the upstream
   Mercury conventional-commit style: `fix:`, `feat:`,
...
Cortex hackathon deadlines pass — Mercury Creative May 3, Gemma 4 Good
May 18, both 2026):
...
- [x] Frontend: mobile-first redesign in `mercury-web/`, Mercury Silver
      palette, distinct from upstream Mercury
...
- [x] `mercury_{cli,constants,state,logging,time}` -> `mercury_*`
...
- [ ] Self-evolution skill wrappers (Mercury-self-evolution, ml-intern,
      autoresearch)
- [ ] `mercury_tools` (the auto-generated namespace exposed by the
...
- [ ] `argparse` prog name in `mercury_cli` — CLI usage line still
      prints "mercury" (cosmetic)
- [ ] `MERCURY_*` env-var renames pending a config-migration story
...
## Gmail MCP (project-level)

4 Gmail accounts available via `.mcp.json` at repo root. Each server is
`@gongrzhe/server-gmail-autoauth-mcp` with per-account credentials at
`C:\Users\soumi\.gmail-mcp-accounts\{account}\.gmail-mcp\`.

| Server | Email |
|---|---|
| `gmail-workspace` | soumitlahiri@philanthropytraders.com |
| `gmail-personal` | lahirisoumit@gmail.com |
| `gmail-legacy` | soumitty@gmail.com |
| `gmail-quantum` | quantumphoenix.om@gmail.com |

OAuth client: GCP project `abm-isu`, app "Snowy" (testing mode).
Non-org accounts require test-user listing in GCP Console.
Full skill docs: `~/.claude/skills/gmail-mcp/SKILL.md`.
