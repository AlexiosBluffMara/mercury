...
Thank you for contributing to Mercury Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Mercury to work everywhere.
...
Bundled skills (in `skills/`) ship with every Mercury install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `mercury skills browse` (labeled "official") and install it with `mercury skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `mercury skills install`.
...
```bash
mkdir -p ~/.mercury/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.mercury/config.yaml
touch ~/.mercury/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.mercury/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/mercury" ~/.local/bin/mercury

# Verify
mercury doctor
mercury chat -q "Hello"
```
...
├── cli.py                    # MercuryCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (mercury-cli, mercury-telegram, etc.)
├── mercury_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── mercury_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.mercury/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (mercury-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.mercury/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.mercury/`)

| Path | Purpose |
|------|---------|
| `~/.mercury/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.mercury/.env` | API keys and secrets |
| `~/.mercury/auth.json` | OAuth credentials (Nous Portal) |
| `~/.mercury/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.mercury/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.mercury/state.db` | SQLite session database |
| `~/.mercury/sessions/` | JSON session logs |
| `~/.mercury/cron/` | Scheduled job data |
| `~/.mercury/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`mercury_state.py`) with full-text search and unique session titles. JSON logs go to `~/.mercury/sessions/`.
...
Four fields are supported under `metadata.mercury`:
...
The user may skip setup and keep loading the skill. Mercury only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `mercury setup` or update `~/.mercury/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Mercury tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `mercury --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Mercury uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.mercury/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `mercury_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `mercury_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Mercury runs on Linux, macOS, and WSL2 on Windows. When writing code that touches the OS:
...
Mercury has terminal access. Security matters.
...
2. **Test manually**: Run `mercury` and exercise the code path you changed
...
- Include: OS, Python version, Mercury version (`mercury version`), full error traceback
