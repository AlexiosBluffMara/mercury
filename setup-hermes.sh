...
# 5. Symlinks the 'mercury' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Mercury Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink mercury into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up mercury command..."

MERCURY_BIN="$SCRIPT_DIR/venv/bin/mercury"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$MERCURY_BIN" "$COMMAND_LINK_DIR/mercury"
echo -e "${GREEN}✓${NC} Symlinked mercury → $COMMAND_LINK_DISPLAY_DIR/mercury"
...
# ============================================================================
# Seed bundled skills into ~/.mercury/skills/
# ============================================================================

MERCURY_SKILLS_DIR="${MERCURY_HOME:-$HOME/.mercury}/skills"
mkdir -p "$MERCURY_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.mercury/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$MERCURY_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  mercury status        # Check configuration"
if is_termux; then
    echo "  mercury gateway       # Run gateway in foreground"
else
    echo "  mercury gateway install # Install gateway service (messaging + cron)"
fi
echo "  mercury cron list     # View scheduled jobs"
echo "  mercury doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m mercury_cli.main setup
fi
