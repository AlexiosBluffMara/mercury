...
"""
    ...
    def test_gateway_still_loads_skill_but_returns_setup_guidance(
        ...
        with patch.dict(
            os.environ, {"MERCURY_SESSION_PLATFORM": "telegram"}, clear=False
        ):
            with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
                _make_skill(
                    tmp_path,
...
class TestTemplateVarSubstitution:
    """``${MERCURY_SKILL_DIR}`` and ``${MERCURY_SESSION_ID}`` in SKILL.md body
    ...
    def test_substitutes_skill_dir(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            skill_dir = _make_skill(
                tmp_path,
                "templated",
                body="Run: node ${MERCURY_SKILL_DIR}/scripts/foo.js",
            )
            scan_skill_commands()
        ...
        assert "${MERCURY_SKILL_DIR}" not in msg.split("[Skill directory:")[0]
    ...
    def test_substitutes_session_id_when_available(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            _make_skill(
                tmp_path,
                "sess-templated",
                body="Session: ${MERCURY_SESSION_ID}",
            )
            scan_skill_commands()
    ...
    def test_leaves_session_id_token_when_missing(self, tmp_path):
        ...
        assert "Session: ${MERCURY_SESSION_ID}" in msg
    ...
    def test_disable_template_vars_via_config(self, tmp_path):
        ...
        assert "${MERCURY_SKILL_DIR}/scripts/foo.js" in msg
