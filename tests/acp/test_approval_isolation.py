...
- 96vc: ACP didn't set MERCURY_EXEC_ASK, so `check_all_command_guards`
...
1. Setting MERCURY_EXEC_ASK inside _run_agent (wraps the agent call).
...
class TestAcpExecAskGate:
    """GHSA-96vc-wcxf-jjff: ACP's _run_agent must set MERCURY_INTERACTIVE so
    ...
    (MERCURY_EXEC_ASK takes the gateway-queue path which requires a
    ...
    def test_interactive_env_var_routes_to_callback(self, monkeypatch):
        """When MERCURY_INTERACTIVE is set and an approval callback is
        ...
        # Without MERCURY_INTERACTIVE: takes auto-approve path, callback NOT called
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert result["approved"] is True
        assert called_with == [], (
            "without MERCURY_INTERACTIVE the non-interactive auto-approve "
            "path should fire without consulting the callback"
        )

        # With MERCURY_INTERACTIVE: callback IS called, approval flows through it
        monkeypatch.setenv("MERCURY_INTERACTIVE", "1")
        called_with.clear()
        result = check_all_command_guards(
            "rm -rf /tmp/test-exec-ask", "local", approval_callback=fake_cb,
        )
        assert called_with, (
            "with MERCURY_INTERACTIVE the approval path should consult the "
