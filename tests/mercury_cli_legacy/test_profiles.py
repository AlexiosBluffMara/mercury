"""Comprehensive tests for mercury_cli.profiles module.
...
from mercury_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_mercury_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and MERCURY_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * MERCURY_HOME  -> tmp_path/.mercury  (so get_mercury_home() agrees)
    * Creates the bare-minimum ~/.mercury directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".mercury"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("MERCURY_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_mercury_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".mercury" / "profiles" / "coder"
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("mercury_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".mercury" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_mercury_home_returns_default(self, profile_env):
        # MERCURY_HOME points to tmp_path/.mercury which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom MERCURY_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("MERCURY_HOME", str(custom))
        # With Docker-aware roots, a custom MERCURY_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("mercury")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("mercury_cli.profiles.check_alias_collision", return_value="skip"):
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("mercury-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".mercury_history",
        ...
        excluded_prefixes = [
            "default/mercury-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.mercury_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_mercury_profiles_function(self):
        script = generate_bash_completion()
        assert "_mercury_profiles" in script

    def test_zsh_completion_has_mercury_function(self):
        script = generate_zsh_completion()
        assert "_mercury" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultMercuryHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_mercury_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".mercury" / "profiles"

    def test_default_mercury_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_mercury_home()
        assert home == tmp_path / ".mercury"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (MERCURY_HOME outside ~/.mercury), profiles go under MERCURY_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MERCURY_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_mercury_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_mercury_home() returns MERCURY_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MERCURY_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (MERCURY_HOME under ~/.mercury), profiles root is still ~/.mercury/profiles."""
        native = tmp_path / ".mercury"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under MERCURY_HOME."""
        from mercury_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MERCURY_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under MERCURY_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MERCURY_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MERCURY_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".mercury" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from mercury_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".mercury"
