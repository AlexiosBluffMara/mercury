...
MERCURY_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot") while
_PROVIDER_MODELS and config.yaml use Mercury IDs ("copilot").  The slug
...
def test_copilot_uses_mercury_slug():
...
def test_kimi_for_coding_alias():
    ...
    from mercury_cli.auth import resolve_provider
...
def test_kimi_for_coding_overlay_uses_mercury_slug():
...
def test_kilo_overlay_uses_mercury_slug():
...
def test_mapped_provider_credential_pool_visibility(monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.auth._load_auth_store",
