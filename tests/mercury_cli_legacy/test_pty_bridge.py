"""Unit tests for mercury_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf mercury-ok"])
        try:
            output = _read_until(bridge, b"mercury-ok")
            assert b"mercury-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$MERCURY_PTY_TEST\""],
            env={**os.environ, "MERCURY_PTY_TEST": "pty-env-works"},
        )
        try:
