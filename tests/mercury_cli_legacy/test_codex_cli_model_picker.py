...
Covers:
 - Normal path (tokens already in Mercury auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Mercury
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `mercury auth openai-codex`. The old
...
@pytest.fixture()
def mercury_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Mercury auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(mercury_auth_only_env):
    """openai-codex appears when tokens are already in Mercury auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Mercury
    auth store."""
    mercury_home = tmp_path / ".mercury"
    mercury_home.mkdir()
    ...
    (mercury_home / "auth.json").write_text(
