"""Tests for mercury_cli.web_server and related config utilities."""
...
class TestReloadEnv:
    ...
    def test_adds_new_vars(self, tmp_path):
        ...
        with patch("mercury_cli.config.get_env_path", return_value=env_file):
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Mercury vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Mercury env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch("mercury_cli.config.get_env_path", return_value=env_file):
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_mercury_home):
        """Create a TestClient and isolate the state DB under the test MERCURY_HOME."""
        try:
        ...
        from mercury_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(mercury_state, "DEFAULT_DB_PATH", get_mercury_home() / "state.db")
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from mercury_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_no_token(self, tmp_path):
        ...
        from starlette.testclient import TestClient
        from mercury_cli.web_server import app
        from mercury_cli.config import save_env_value
    ...
    def test_reveal_env_var_bad_token(self, tmp_path):
        ...
        from mercury_cli.config import save_env_value
        from mercury_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from mercury_cli.config import save_env_value
        from mercury_cli.web_server import _SESSION_TOKEN
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/mercury_cli/web_server.py")
        assert resp.status_code in (200, 404)
        if resp.status_code == 200:
...
class TestBuildSchemaFromConfig:
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from mercury_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_mercury_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import mercury_cli.skills_config as skills_config
        import mercury_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import mercury_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from mercury_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from mercury_cli.web_server import _denormalize_config_from_web
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from mercury_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.mercury/dashboard-themes/."""
...
# or ``sh -c 'printf …'``) instead of the real ``mercury --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"mercury-ws-ok" in buf:
                    break
            assert b"mercury-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from mercury_cli.pty_bridge import PtyUnavailableError
        ...
        import mercury_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        MERCURY_TUI_SIDECAR_URL env var pointing back at /api/pub on the
