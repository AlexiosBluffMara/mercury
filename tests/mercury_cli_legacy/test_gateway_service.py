...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "mercury-gateway.service"
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("MERCURY_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("MERCURY_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("MERCURY_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_mercury_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: mercury gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup mercury gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.mercury.gateway"])
            ),
        )
...
class TestSystemUnitMercuryHome:
    """MERCURY_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'MERCURY_HOME=/home/alice/.mercury' in unit
        assert '/root/.mercury' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: MERCURY_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("MERCURY_HOME", "/root/.mercury/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'MERCURY_HOME=/home/alice/.mercury/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_mercury_home(self, monkeypatch):
        # Custom MERCURY_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("MERCURY_HOME", "/opt/mercury-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'MERCURY_HOME=/opt/mercury-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's MERCURY_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        mercury_home = str(gateway_cli.get_mercury_home().resolve())
        assert f'MERCURY_HOME={mercury_home}' in unit


class TestMercuryHomeForTargetUser:
    """Unit tests for _mercury_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("MERCURY_HOME", raising=False)

        result = gateway_cli._mercury_home_for_target_user("/home/alice")
        assert result == "/home/alice/.mercury"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._mercury_home_for_target_user("/home/alice")
        assert result == "/home/alice/.mercury/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("MERCURY_HOME", "/opt/mercury")

        result = gateway_cli._mercury_home_for_target_user("/home/alice")
        assert result == "/opt/mercury"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "mercury gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_mercury_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.mercury should not produce a --profile flag."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        result = gateway_cli._profile_arg(str(mercury_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.mercury/profiles/mybot should return '--profile mybot'."""
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile MERCURY_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "mercury"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.mercury/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".mercury" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".mercury" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("MERCURY_HOME", str(profile_dir))
        monkeypatch.setattr(gateway_cli, "get_mercury_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".mercury" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.mercury.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".mercury" / "mercury-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".mercury" / "mercury-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/mercury", str(tmp_path / "alice"))
        assert result == "/opt/mercury"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".mercury" / "mercury-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".mercury" / "mercury-agent"
        ...
        monkeypatch.setattr(gateway_cli, "get_mercury_home", lambda: root_home / ".mercury")
        ...
        assert "WorkingDirectory=/home/alice/.mercury/mercury-agent" in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "mercury-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "mercury-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'mercury gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'mercury gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'mercury gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "mercury gateway run" in out


class TestLegacyMercuryUnitDetection:
    """Tests for _find_legacy_mercury_units / has_legacy_mercury_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``mercury.service`` unit behind when the service was
    renamed to ``mercury-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``mercury.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``mercury-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Mercury Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m mercury_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_mercury_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "mercury.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_mercury_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "mercury.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_mercury_units() is True

    def test_detects_legacy_mercury_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "mercury.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_mercury_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``mercury-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "mercury-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "mercury-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "mercury-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_mercury_units() is False

    def test_ignores_unrelated_mercury_service(self, tmp_path, monkeypatch):
        """Third-party ``mercury.service`` that isn't ours stays untouched.

        If a user has some other package named ``mercury`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "mercury.service").write_text(
            "[Unit]\nDescription=Some Other Mercury\n[Service]\n"
            "ExecStart=/opt/other-mercury/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_mercury_units() == []
        assert gateway_cli.has_legacy_mercury_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "mercury.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "mercury.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m mercury_cli.main gateway run
          - python path/to/mercury_cli/main.py gateway run
          - mercury gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m mercury_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/mercury/mercury_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/mercury gateway run --replace",
            "ExecStart=/venv/bin/python /opt/mercury/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = f"mercury.service" if i == 0 else f"mercury.service"  # same name
            # Test each variant fresh
            (user_dir / "mercury.service").write_text(
                f"[Unit]\nDescription=Old Mercury\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_mercury_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "mercury.service" in out
        assert "mercury gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "mercury.service"
...
class TestRemoveLegacyMercuryUnits:
    """Tests for remove_legacy_mercury_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_mercury_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_mercury_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop mercury.service" in c for c in cmds_joined)
        assert any("--user disable mercury.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo mercury gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop mercury.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable mercury.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "mercury.service"
        system_legacy = system_dir / "mercury.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (mercury-gateway-coder.service)
        ...
        profile_unit = user_dir / "mercury-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "mercury-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_mercury_units(interactive=True)
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import mercury_cli.main as cli_main
        ...
        # public, just confirm that `mercury gateway --help` shows it.
        import subprocess
        import sys
        ...
        result = subprocess.run(
            [sys.executable, "-m", "mercury_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_mercury_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "mercury_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_mercury_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_mercury_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_mercury_units", fake_remove)
