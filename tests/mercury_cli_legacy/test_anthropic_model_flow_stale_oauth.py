"""Tests for Bug #12905 fix — stale OAuth token detection in mercury model flow.

Bug 3: `mercury model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.mercury/.env but no valid
...
from mercury_cli.config import load_env, save_env_value
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        from mercury_cli.main import _model_flow_anthropic
        cfg = {}
