...
import mercury_cli.doctor as doctor
import mercury_cli.gateway as gateway_cli
from mercury_cli import doctor as doctor_mod
from mercury_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "MERCURY_HOME", mercury_home)
    monkeypatch.delenv("MERCURY_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("MERCURY_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_mercury_home(self, tmp_path, provider=""):
        """Create a minimal MERCURY_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_mercury_home(tmp_path, provider)
