...
``mercury_cli.runtime_provider.resolve_runtime_provider`` already strips
``/v1`` at fresh agent init (PR #4918), but the ``/model`` mid-session
switch path in ``mercury_cli.model_switch.switch_model`` was missing the
...
def _run_opencode_switch(
    ...
    with (
        patch("mercury_cli.model_switch.resolve_alias", return_value=None),
        patch("mercury_cli.model_switch.list_provider_models", return_value=[]),
        patch(
        ...
        patch(
            "mercury_cli.models.validate_requested_model",
            return_value=_MOCK_VALIDATION,
        ),
        patch("mercury_cli.model_switch.get_model_info", return_value=None),
        patch("mercury_cli.model_switch.get_model_capabilities", return_value=None),
        patch("mercury_cli.models.detect_provider_for_model", return_value=None),
    ):
        return switch_model(
            raw_input=raw_input,
...
class TestStaleConfigDefaultDoesNotWedgeResolver:
    ...
    def test_kimi_switch_keeps_v1_despite_claude_config_default(self, tmp_path, monkeypatch):
        ...
        # Re-import with the new MERCURY_HOME so config cache is fresh.
        import mercury_cli.config as _cfg_mod
        importlib.reload(_cfg_mod)
        import mercury_cli.runtime_provider as _rp_mod
        importlib.reload(_rp_mod)
        import mercury_cli.model_switch as _ms_mod
