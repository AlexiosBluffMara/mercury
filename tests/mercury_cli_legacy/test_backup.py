"""Tests for mercury backup and import commands."""
...
def _make_mercury_tree(root: Path) -> None:
    """Create a realistic ~/.mercury directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "mercury_state.db").write_bytes(b"fake-state")
    ...
    # mercury-agent repo (should be EXCLUDED)
    (root / "mercury-agent").mkdir(exist_ok=True)
    (root / "mercury-agent" / "run_agent.py").write_text("# big file\n")
    (root / "mercury-agent" / ".git").mkdir()
    (root / "mercury-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_mercury_agent(self):
        from mercury_cli.backup import _should_exclude
        assert _should_exclude(Path("mercury-agent/run_agent.py"))
        assert _should_exclude(Path("mercury-agent/.git/HEAD"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        mercury_home.mkdir()
        _make_mercury_tree(mercury_home)

        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        # get_default_mercury_root needs this
    ...
    def test_excludes_mercury_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include mercury-agent/ directory."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        _make_mercury_tree(mercury_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "mercury-agent" in n]
            assert agent_files == [], f"mercury-agent files leaked into backup: {agent_files}"
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/mercury-backup-*.zip."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        (mercury_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("mercury-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Mercury backup."""
        from mercury_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only mercury_state.db (old wrong name) is rejected."""
        from mercury_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["mercury_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into mercury home."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (mercury_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (mercury_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (mercury_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (mercury_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_mercury_prefix(self, tmp_path, monkeypatch):
        """Import strips .mercury/ prefix if all entries share it."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".mercury/config.yaml": "model: test\n",
            ".mercury/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (mercury_home / "config.yaml").read_text() == "model: test\n"
        assert (mercury_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_mercury_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a mercury backup."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (mercury_home / "config.yaml").exists()
        # traversal file should NOT exist outside mercury home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (mercury_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (mercury_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (mercury_home / "config.yaml").read_text() == "model: restored\n"
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".mercury"
        src_home.mkdir(parents=True)
        _make_mercury_tree(src_home)

        monkeypatch.setenv("MERCURY_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from mercury_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".mercury"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("MERCURY_HOME", str(dst_home))
        ...
        # mercury-agent should NOT be present
        assert not (dst_home / "mercury-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from mercury_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without mercury markers fails validation."""
        import io
        from mercury_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_mercury(self):
        """Detects .mercury/ prefix wrapping all entries."""
        import io
        from mercury_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".mercury/config.yaml", "test")
            zf.writestr(".mercury/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".mercury/"
...
class TestBackupEdgeCases:
    def test_nonexistent_mercury_home(self, tmp_path, monkeypatch):
        """Backup exits when mercury home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".mercury"
        monkeypatch.setenv("MERCURY_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("mercury-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_mercury_home(self, tmp_path, monkeypatch):
        """Backup handles empty mercury home (no files to back up)."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        # Only excluded dirs, no actual files
        (mercury_home / "__pycache__").mkdir()
        (mercury_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = mercury_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = mercury_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_mercury(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside mercury root."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        (mercury_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside mercury home
        out_zip = mercury_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        (mercury_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        (mercury_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_permission_error_during_import(self, tmp_path, monkeypatch):
        ...
        locked_dir = mercury_home / "locked"
        locked_dir.mkdir()
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (mercury_home / "config.yaml").exists()
        assert (mercury_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (mercury_home / "profiles" / "coder" / "config.yaml").exists()
        assert (mercury_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "mercury -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import mercury_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "mercury_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from mercury_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def mercury_home(self, tmp_path):
        """Create a fake MERCURY_HOME with critical state files."""
    ...
    def test_creates_snapshot(self, mercury_home):
        from mercury_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(mercury_home=mercury_home)
        assert snap_id is not None
        snap_dir = mercury_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, mercury_home):
        from mercury_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", mercury_home=mercury_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, mercury_home):
        from mercury_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(mercury_home=mercury_home)
        db_copy = mercury_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, mercury_home):
        from mercury_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(mercury_home=mercury_home)
        assert (mercury_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, mercury_home):
        from mercury_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(mercury_home=mercury_home)
        with open(mercury_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(mercury_home=empty) is None

    def test_list_snapshots(self, mercury_home):
        from mercury_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", mercury_home=mercury_home)
        id2 = create_quick_snapshot(label="second", mercury_home=mercury_home)

        snaps = list_quick_snapshots(mercury_home=mercury_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, mercury_home):
        from mercury_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", mercury_home=mercury_home)
        snaps = list_quick_snapshots(limit=3, mercury_home=mercury_home)
        assert len(snaps) == 3

    def test_restore_config(self, mercury_home):
        from mercury_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(mercury_home=mercury_home)

        (mercury_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (mercury_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, mercury_home=mercury_home)
        assert result is True
        assert "openrouter" in (mercury_home / "config.yaml").read_text()

    def test_restore_state_db(self, mercury_home):
        from mercury_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(mercury_home=mercury_home)

        conn = sqlite3.connect(str(mercury_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, mercury_home=mercury_home)
    ...
    def test_restore_nonexistent(self, mercury_home):
        from mercury_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", mercury_home=mercury_home) is False

    def test_auto_prune(self, mercury_home):
        from mercury_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", mercury_home=mercury_home)
        snaps = list_quick_snapshots(limit=100, mercury_home=mercury_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, mercury_home):
        from mercury_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", mercury_home=mercury_home)
        deleted = prune_quick_snapshots(keep=3, mercury_home=mercury_home)
        assert deleted == 7
        assert len(list_quick_snapshots(mercury_home=mercury_home)) == 3
