...
def main_mod(monkeypatch):
    import mercury_cli.main as mod
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(sys.modules, "mercury_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB()))

    main_mod._print_tui_exit_summary("20260409_000001_abc123")
    out = capsys.readouterr().out

    assert "Resume this session with:" in out
    assert "mercury --tui --resume 20260409_000001_abc123" in out
    assert 'mercury --tui -c "demo title"' in out
