...
from mercury_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from mercury_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("MERCURY_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_mercury_ca_bundle_env_falls_back(self, monkeypatch):
        from mercury_cli.auth import _resolve_verify

        monkeypatch.setenv("MERCURY_CA_BUNDLE", "/nonexistent/mercury-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (mercury_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from mercury_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    mercury_home = tmp_path / "mercury"
    _setup_nous_auth(mercury_home, access_token="at-123")
    monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    mercury_home = tmp_path / "mercury"
    _setup_nous_auth(mercury_home, access_token="at-fresh")
    monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mercury_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    mercury_home = tmp_path / "mercury"
    _setup_nous_auth(mercury_home, refresh_token="refresh-old")
    monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
    ...
    monkeypatch.setattr("mercury_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("mercury_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `mercury model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        auth_path = mercury_home / "auth.json"
        auth_path.write_text(json.dumps({
            "version": 1,
            "active_provider": "openrouter",
            "providers": {"openrouter": {"api_key": "sk-or-fake"}},
        }))
        return mercury_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import mercury_cli.auth as auth_mod
        import mercury_cli.models as models_mod
        import mercury_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from mercury_cli.auth import PROVIDER_REGISTRY, _login_nous

        mercury_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `mercury auth add nous`
    ...
    """
    from mercury_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Mercury is not logged into Nous Portal").
    ...
    """
    from mercury_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
    ...
    # doesn't raise "Mercury is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `mercury auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Mercury must surface an
    ...
    re-authenticate" string led users to report this as a Mercury persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from mercury_cli.auth import _refresh_access_token
    ...
    assert "mercury auth add nous" in message.lower()
