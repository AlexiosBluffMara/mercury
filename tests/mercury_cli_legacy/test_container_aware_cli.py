...
def container_env(tmp_path, monkeypatch):
    """Set up a fake MERCURY_HOME with .container-mode file."""
    mercury_home = tmp_path / ".mercury"
    mercury_home.mkdir()
    monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
    monkeypatch.delenv("MERCURY_DEV", raising=False)

    container_mode = mercury_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=mercury-agent\n"
        "exec_user=mercury\n"
        "mercury_bin=/data/current-package/bin/mercury\n"
    )
    return mercury_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("mercury_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "mercury-agent"
    assert info["exec_user"] == "mercury"
    assert info["mercury_bin"] == "/data/current-package/bin/mercury"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("mercury_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_mercury_dev(container_env, monkeypatch):
    """Returns None when MERCURY_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("MERCURY_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_mercury_dev_zero(container_env, monkeypatch):
    """MERCURY_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("MERCURY_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        mercury_home = Path(tmpdir) / ".mercury"
        mercury_home.mkdir()
        (mercury_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("mercury_constants.is_container", return_value=False), \
             patch("mercury_cli.config.get_mercury_home", return_value=mercury_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MERCURY_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=mercury-custom\n"
        "exec_user=myuser\n"
        "mercury_bin=/opt/mercury/bin/mercury\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "mercury-custom"
    assert info["exec_user"] == "myuser"
    assert info["mercury_bin"] == "/opt/mercury/bin/mercury"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from mercury_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "mercury"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "mercury-agent" in cmd
    assert "/data/current-package/bin/mercury" in cmd
    assert "chat" in cmd
