...
Ensures ``mercury update`` correctly detects running gateways managed by
...
from mercury_cli.main import cmd_update
...
def _no_restart_verify_sleep(monkeypatch):
    """mercury_cli/main.py uses time.sleep(3) after systemctl restart to
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units mercury-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="mercury-gateway.service loaded active running Mercury Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>MERCURY_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: mercury gateway run'."""
        ...
        assert "Restart manually: mercury gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: mercury gateway run" in captured
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted mercury-gateway" in captured
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "systemctl" in joined and "list-units" in joined:
                if "--user" in joined:
                    return subprocess.CompletedProcess(
                        cmd, 0,
                        stdout="mercury-gateway.service loaded active running\n",
                        stderr="",
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "mercury_cli.gateway._graceful_restart_via_sigusr1",
...
class TestServicePidExclusion:
    ...
    where ``mercury update`` would restart the gateway and immediately kill it.
    """
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="mercury-gateway.service loaded inactive dead Mercury Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.mercury/mercury-agent/venv/bin/python -m mercury_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.mercury/mercury-agent/venv/bin/python -m mercury_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
        ...
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda mercury_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``mercury update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point MERCURY_HOME at a temp dir so the marker file lands there
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        import mercury_cli.config as _cfg
        monkeypatch.setattr(_cfg, "get_mercury_home", lambda: mercury_home)
        # Also patch the module-level ref used by cmd_update
        import mercury_cli.main as _main_mod
        monkeypatch.setattr(_main_mod, "get_mercury_home", lambda: mercury_home)
        ...
        exit_code_path = mercury_home / ".update_exit_code"
...
class TestCmdUpdateLegacyGatewayWarning:
    """Tests for the legacy mercury.service warning printed by `mercury update`.

    Users who installed Mercury before the service rename often have a
    dormant ``mercury.service`` that starts flap-fighting the current
    ``mercury-gateway.service`` after PR #5646. Every ``mercury update``
    should remind them to run ``mercury gateway migrate-legacy`` until
    they do.
    """
    ...
    def test_update_prints_legacy_warning_when_detected(
        ...
        user_dir.mkdir()
        system_dir.mkdir()
        legacy_path = user_dir / "mercury.service"
        ...
        captured = capsys.readouterr().out
        assert "Legacy Mercury gateway unit(s) detected" in captured
        assert "mercury.service" in captured
        assert "mercury gateway migrate-legacy" in captured
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Mercury gateway" not in captured
    ...
    def test_update_does_not_flag_profile_units(
        ...
        """Profile units (mercury-gateway-coder.service) must not trigger the warning.

        This is the core safety invariant: the legacy allowlist is
        ``mercury.service`` only, no globs.
        """
        user_dir = tmp_path / "user"
        system_dir = tmp_path / "system"
        user_dir.mkdir()
        ...
        captured = capsys.readouterr().out
        assert "Legacy Mercury gateway" not in captured
        assert "mercury-gateway-coder.service" not in captured  # not flagged
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Mercury gateway" in captured
