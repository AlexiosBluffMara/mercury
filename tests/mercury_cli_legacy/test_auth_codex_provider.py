"""Tests for Codex auth — tokens stored in Mercury auth store (~/.mercury/auth.json)."""
...
def _setup_mercury_auth(mercury_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Mercury auth store."""
    ...
    auth_file = mercury_home / "auth.json"
...
def test_read_codex_tokens_success(tmp_path, monkeypatch):
    ...
    _setup_mercury_auth(mercury_home)
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    mercury_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (mercury_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    mercury_home = tmp_path / "mercury"
    _setup_mercury_auth(mercury_home, access_token="")
    monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_mercury_auth(mercury_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
    ...
    monkeypatch.setattr("mercury_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    mercury_home = tmp_path / "mercury"
    _setup_mercury_auth(mercury_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Mercury auth store, not ~/.codex/."""
    mercury_home = tmp_path / "mercury"
    codex_home = tmp_path / "codex-cli"
    mercury_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "mercury-at", "refresh_token": "mercury-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Mercury store
    assert not (codex_home / "auth.json").exists()

    # Mercury auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "mercury-at"


def test_resolve_returns_mercury_auth_store_source(tmp_path, monkeypatch):
    mercury_home = tmp_path / "mercury"
    _setup_mercury_auth(mercury_home)
    monkeypatch.setenv("MERCURY_HOME", str(mercury_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "mercury-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("mercury_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "mercury_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "mercury_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("mercury_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("mercury_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
