...
def test_mercury_provider_subclass_exists():
    """MercuryMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _MERCURY_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _MERCURY_PROVIDER_CLS is not None
    assert issubclass(_MERCURY_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_mercury_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns MercuryMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _MERCURY_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("MERCURY_HOME", str(tmp_path))
    ...
    assert _MERCURY_PROVIDER_CLS is not None
    assert isinstance(provider, _MERCURY_PROVIDER_CLS)
    assert provider._mercury_server_name == "srv"
