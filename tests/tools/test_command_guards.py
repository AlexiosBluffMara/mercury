...
def _clean_state():
    ...
    saved = {}
    for k in ("MERCURY_INTERACTIVE", "MERCURY_GATEWAY_SESSION", "MERCURY_EXEC_ASK", "MERCURY_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["MERCURY_INTERACTIVE"] = "1"
        session_key = os.getenv("MERCURY_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No MERCURY_INTERACTIVE or MERCURY_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
