...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        mercury_home.mkdir()
        (mercury_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"MERCURY_HOME": str(mercury_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(mercury_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.mercury/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        (mercury_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        (mercury_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        (mercury_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.mercury/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        mercury_home = tmp_path / ".mercury"
        (mercury_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"MERCURY_HOME": str(mercury_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.mercury")

        assert mounts[0]["container_path"] == "/home/user/.mercury/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.mercury/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.mercury/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape MERCURY_HOME."""
        monkeypatch.setenv("MERCURY_HOME", str(tmp_path / ".mercury"))
        (tmp_path / ".mercury").mkdir()

        # Create a sensitive file one level above mercury_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside mercury_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside MERCURY_HOME must still be registered."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        (mercury_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_mercury_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of MERCURY_HOME must be allowed."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        subdir = mercury_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside MERCURY_HOME pointing outside must be rejected."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))

        # Create a sensitive file outside mercury_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside mercury_home pointing outside
        symlink = mercury_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes MERCURY_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, mercury_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape MERCURY_HOME."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        ...
        sensitive.write_text("{}")
        self._write_config(mercury_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(mercury_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside MERCURY_HOME via config must still mount."""
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))

        (mercury_home / "oauth.json").write_text("{}")
        self._write_config(mercury_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        (mercury_home / "cache" / "documents").mkdir(parents=True)
        (mercury_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.mercury/cache/documents" in paths
        assert "/root/.mercury/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.mercury/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        # Use legacy dir name — get_mercury_dir prefers old if it exists
        (mercury_home / "document_cache").mkdir()
        (mercury_home / "image_cache").mkdir()
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(mercury_home / "document_cache") in host_paths
        assert str(mercury_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.mercury/cache/documents" in container_paths
        assert "/root/.mercury/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        doc_dir = mercury_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        ss_dir = mercury_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.mercury/cache/screenshots/session_abc/screen1.png"
