...
class TestWriteDenyExactPaths:
    ...
    def test_mercury_env(self):
        # ``.env`` under the active MERCURY_HOME (profile-aware, not just
        # ``~/.mercury``) must be write-denied. The hermetic test conftest
        # points MERCURY_HOME at a tempdir — resolve via get_mercury_home()
        # to match the denylist.
        from mercury_constants import get_mercury_home
        path = str(get_mercury_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_mercury_config_not_env(self):
        path = os.path.join(str(Path.home()), ".mercury", "config.yaml")
