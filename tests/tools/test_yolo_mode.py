"""Tests for --yolo (MERCURY_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When MERCURY_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("MERCURY_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With MERCURY_YOLO_MODE, dangerous commands are auto-approved."""
    ...
    def test_yolo_mode_not_set_by_default(self):
        """MERCURY_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("MERCURY_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for MERCURY_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("MERCURY_YOLO_MODE", "")
        monkeypatch.setenv("MERCURY_INTERACTIVE", "1")
        monkeypatch.setenv("MERCURY_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("MERCURY_YOLO_MODE") returns ""
