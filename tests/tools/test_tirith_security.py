...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._mercury_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_mercury_bin(self):
        """After _INSTALL_FAILED, manual install in MERCURY_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        mercury_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(mercury_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(mercury_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._mercury_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == mercury_bin
            assert _tirith_mod._resolved_path == mercury_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# MERCURY_HOME isolation
# ---------------------------------------------------------------------------

class TestMercuryHomeIsolation:
    def test_mercury_bin_dir_respects_mercury_home(self):
        """_mercury_bin_dir must use MERCURY_HOME, not hardcoded ~/.mercury."""
        from tools.tirith_security import _mercury_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"MERCURY_HOME": tmpdir}):
            result = _mercury_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_mercury_home(self):
        """_failure_marker_path must use MERCURY_HOME, not hardcoded ~/.mercury."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"MERCURY_HOME": "/custom/mercury"}):
            result = _failure_marker_path()
        assert result == "/custom/mercury/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets MERCURY_HOME; verify it's active."""
        mercury_home = os.getenv("MERCURY_HOME")
        assert mercury_home is not None, "MERCURY_HOME should be set by conftest"
        assert "mercury_test" in mercury_home, "Should point to test temp dir"

    def test_get_mercury_home_fallback(self):
        """Without MERCURY_HOME set, falls back to ~/.mercury."""
        from tools.tirith_security import _get_mercury_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove MERCURY_HOME entirely
            os.environ.pop("MERCURY_HOME", None)
            result = _get_mercury_home()
        assert result == os.path.join(os.path.expanduser("~"), ".mercury")
