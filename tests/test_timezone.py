"""
Tests for timezone support (mercury_time module + integration points).
...
def _reset_mercury_time_cache():
    """Reset the mercury_time module cache (replacement for removed reset_cache)."""
    mercury_time._cached_tz = None
    mercury_time._cached_tz_name = None
    mercury_time._cache_resolved = False


# =========================================================================
# mercury_time.now() — core helper
# =========================================================================

class TestMercuryTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_mercury_time_cache()

    def teardown_method(self):
        _reset_mercury_time_cache()
        os.environ.pop("MERCURY_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["MERCURY_TIMEZONE"] = "Asia/Kolkata"
        result = mercury_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["MERCURY_TIMEZONE"] = "UTC"
        result = mercury_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["MERCURY_TIMEZONE"] = "America/New_York"
        result = mercury_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["MERCURY_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="mercury_time"):
            result = mercury_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["MERCURY_TIMEZONE"] = "Europe/London"
        tz = mercury_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["MERCURY_TIMEZONE"] = "Not/A/Timezone"
        tz = mercury_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When MERCURY_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) MERCURY_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("MERCURY_TIMEZONE=" + os.environ.get("MERCURY_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "MERCURY_TIMEZONE=NOT_SET" in result["output"], (
            "MERCURY_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When MERCURY_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("MERCURY_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=mercury_tz) which shifted
        absolute time when system-local tz != Mercury tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_mercury_tz(self):
        """Already-aware datetimes should be normalized to Mercury tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Mercury tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Mercury tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Mercury is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Mercury tz
        ...
        # Use a Mercury timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _mercury_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["MERCURY_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_mercury_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Mercury timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["MERCURY_TIMEZONE"] = "US/Eastern"
        _reset_mercury_time_cache()
