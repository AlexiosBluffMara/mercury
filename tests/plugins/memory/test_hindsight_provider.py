...
def provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_mercury_home", lambda: tmp_path
    )

    p = HindsightMemoryProvider()
    p.initialize(session_id="test-session", mercury_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:mercury-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:mercury-agent",
    ]


def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:mercury-agent"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:mercury-agent"]
...
class TestConfig:
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Mercury Agent and the User"

    def test_custom_config_values(self, provider_with_config):
        p = provider_with_config(
            retain_tags=["tag1", "tag2"],
            retain_source="mercury",
        ...
        assert p._retain_source == "mercury"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_mercury_home",
        ...
        assert cfg["banks"]["mercury"]["bankId"] == "env-bank"
        assert cfg["banks"]["mercury"]["budget"] == "high"
...
class TestPostSetup:
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("mercury_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("mercury_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(mercury_home), {"memory": {}})

        assert saved_configs[-1]["memory"]["provider"] == "hindsight"
        assert (mercury_home / ".env").read_text() == "HINDSIGHT_LLM_API_KEY=sk-local-test\nHINDSIGHT_TIMEOUT=120\n"
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(mercury_home))
        provider.post_setup(str(mercury_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        mercury_env = user_home / ".hindsight" / "profiles" / "mercury.env"
        assert coder_env.exists()
        assert not mercury_env.exists()
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        assert item["context"] == "conversation between Mercury Agent and the User"
        ...
        assert item["metadata"]["source"] == "mercury"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Mercury Agent and the User"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_mercury_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", mercury_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", mercury_home=str(tmp_path), platform="cli")
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("mercury") == "mercury"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "mercury-{user}" becomes "mercury-" -> trimmed to "mercury"
        result = _resolve_bank_id_template(
            "mercury-{user}", fallback="default",
    ...
    def test_resolve_invalid_template_returns_fallback(self):
        ...
        result = _resolve_bank_id_template(
            "mercury-{unknown}", fallback="mercury",
            profile="", workspace="", platform="", user="", session="",
        )
        assert result == "mercury"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "mercury-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="mercury",
        )
        assert p._bank_id == "mercury-coder"
        assert p._bank_id_template == "mercury-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "mercury-" which collapses to "mercury"
        p.initialize(session_id="s1", mercury_home=str(tmp_path), platform="cli")
        assert p._bank_id == "mercury"
