...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure MERCURY_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def _make_provider(self, tmp_path, monkeypatch, api_key="rdb-test-key"):
        ...
        (tmp_path / ".mercury").mkdir(exist_ok=True)
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", mercury_home=str(tmp_path / ".mercury"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", mercury_home=profile_home)
        assert p._client.project == "mercury-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".mercury" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir(exist_ok=True)
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", mercury_home=str(mercury_home))
        return p
