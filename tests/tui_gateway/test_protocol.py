...
def server():
    with patch.dict("sys.modules", {
        "mercury_constants": MagicMock(get_mercury_home=MagicMock(return_value="/tmp/mercury_test")),
        "mercury_cli.env_loader": MagicMock(),
        "mercury_cli.banner": MagicMock(),
        "mercury_state": MagicMock(),
    }):
        import importlib
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/mercury-agent-dev": {"name": "mercury-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "mercury-agent-dev", "session_id": sid},
        })
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "mercury-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "mercury-agent-dev"
