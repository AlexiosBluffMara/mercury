...
class TestGetSubprocessHome:
    """Unit tests for mercury_constants.get_subprocess_home()."""

    def test_returns_none_when_mercury_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        mercury_home = tmp_path / ".mercury"
        mercury_home.mkdir()
        profile_home = mercury_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("MERCURY_HOME", str(mercury_home))
        from mercury_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".mercury" / "profiles"
        ...
        monkeypatch.setenv("MERCURY_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("MERCURY_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(mercury_home / "home")
    ...
    def test_no_injection_when_mercury_home_unset(self, monkeypatch):
        monkeypatch.delenv("MERCURY_HOME", raising=False)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from mercury_cli.profiles import _PROFILE_DIRS
    ...
    def test_create_profile_bootstraps_home_dir(self, tmp_path, monkeypatch):
        ...
        from mercury_cli.profiles import create_profile
