...
def _run_switch(raw_input: str, current_provider: str = "openrouter") -> str:
    ...
    with patch("mercury_cli.model_switch.resolve_alias", return_value=None), \
         patch("mercury_cli.model_switch.list_provider_models", return_value=[]), \
         patch("mercury_cli.runtime_provider.resolve_runtime_provider",
               return_value={"api_key": "test", "base_url": "", "api_mode": "chat_completions"}), \
         patch("mercury_cli.models.validate_requested_model", return_value=_MOCK_VALIDATION), \
