...
def _isolate(tmp_path, monkeypatch):
    """Isolate MERCURY_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal MercuryCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = MercuryCLI.__new__(MercuryCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["mercury-core"]
        obj.compact = False
