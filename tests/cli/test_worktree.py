...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("mercury/mercury-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned mercury/* and pr-* branches."""

    def test_prunes_orphaned_mercury_branch(self, git_repo):
        """mercury/mercury-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "mercury/mercury-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "mercury/mercury-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "mercury/mercury-deadbeef" in result.stdout
        ...
        assert "mercury/mercury-deadbeef" in orphaned
        ...
        assert "mercury/mercury-deadbeef" not in result.stdout
