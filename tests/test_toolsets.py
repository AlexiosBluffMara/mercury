...
class TestToolsetConsistency:
    ...
    def test_mercury_platforms_share_core_tools(self):
        """All mercury-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on mercury-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["mercury-cli", "mercury-telegram", "mercury-discord", "mercury-whatsapp", "mercury-slack", "mercury-signal", "mercury-homeassistant"]
