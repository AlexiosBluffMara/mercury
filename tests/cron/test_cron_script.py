...
- Path resolution (absolute, relative to MERCURY_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp MERCURY_HOME."""
    mercury_home = tmp_path / ".mercury"
    mercury_home.mkdir()
    (mercury_home / "cron").mkdir()
    (mercury_home / "cron" / "output").mkdir()
    (mercury_home / "scripts").mkdir()
    monkeypatch.setenv("MERCURY_HOME", str(mercury_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "MERCURY_DIR", mercury_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", mercury_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", mercury_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", mercury_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.mercury/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("MERCURY_SESSION_PLATFORM") is None
        assert os.environ.get("MERCURY_SESSION_CHAT_ID") is None
        assert os.environ.get("MERCURY_SESSION_CHAT_NAME") is None
