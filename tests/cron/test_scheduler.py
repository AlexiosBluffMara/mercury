...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._mercury_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("dotenv.load_dotenv"), \
             patch("mercury_state.SessionDB", return_value=fake_db), \
             patch(
                 "mercury_cli.runtime_provider.resolve_runtime_provider",
                 return_value={
                     "api_key": "test-key",
    ...
    def _make_run_job_patches(self, tmp_path):
        ...
        fake_db = MagicMock()
        return fake_db, [
            patch("cron.scheduler._mercury_home", tmp_path),
            patch("cron.scheduler._resolve_origin", return_value=None),
            patch("dotenv.load_dotenv"),
            patch("mercury_state.SessionDB", return_value=fake_db),
            patch(
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(self, tmp_path):
        ...
        resolves them from ``mercury tools`` platform config for ``cron``
        ...
        # The cron default is _MERCURY_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``mercury tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        fake_db, patches = self._make_run_job_patches(tmp_path)
        # Even if the user has ``mercury tools`` configured to enable web+file
        # for cron, the per-job override wins.
        with patches[0], patches[1], patches[2], patches[3], patches[4], \
             patch("run_agent.AIAgent") as mock_agent_cls, \
             patch(
                 "mercury_cli.tools_config._get_platform_tools",
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("MERCURY_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("MERCURY_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("MERCURY_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("MERCURY_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("MERCURY_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("MERCURY_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("MERCURY_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("MERCURY_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("MERCURY_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
...
class TestRunJobSkillBacked:
    ...
    def test_run_job_preserves_credential_file_passthrough_into_worker_thread(self, tmp_path):
        ...
        with patch("cron.scheduler._mercury_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("tools.credential_files._resolve_mercury_home", return_value=tmp_path), \
             patch("dotenv.load_dotenv"), \
             patch("mercury_state.SessionDB", return_value=fake_db), \
             patch(
                 "mercury_cli.runtime_provider.resolve_runtime_provider",
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """MERCURY_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("MERCURY_CRON_MAX_PARALLEL", "1")
        call_times = []
