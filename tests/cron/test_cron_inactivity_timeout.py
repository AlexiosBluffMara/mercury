...
- Unlimited timeout (MERCURY_CRON_TIMEOUT=0)
- Backward compat: MERCURY_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """MERCURY_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """MERCURY_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("MERCURY_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """MERCURY_CRON_TIMEOUT=0 yields None (unlimited)."""
...
class TestSysPathOrdering:
    ...
    def test_mercury_time_importable(self):
        """mercury_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _mercury_now
        assert callable(_mercury_now)

    def test_mercury_constants_importable(self):
        """mercury_constants should be importable from cron context."""
        from mercury_constants import get_mercury_home
        assert callable(get_mercury_home)
