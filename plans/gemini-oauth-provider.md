...
## Token Lifecycle
- Store at `~/.mercury/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `mercury_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `mercury_cli/models.py` — Add Gemini model catalog
4. `mercury_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `mercury_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `mercury_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
